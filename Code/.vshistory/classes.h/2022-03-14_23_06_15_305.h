...
struct Perk {
	...
	Perk(int c, std::string n, std::string desc) {
		cost = c;
		name = n;
		description = desc;
	}
...
std::vector<Perk> initPerks() {
	...
	perks.push_back(Perk(3, "Efficient Housing", "Houses provide*GREEN* +3 *GREEN* population space."));
	perks.push_back(Perk(3, "Sustainable Fishing", "*ORANGE*Fish*ORANGE* replenish every turn."));
	perks.push_back(Perk(3, "Forced Levy", "No *GOLD*upkeep*GOLD* for workers and spearmen."));
	perks.push_back(Perk(2, "Sharper Spears", "Spearmen *GREEN*+1*GREEN* ATK."));
	perks.push_back(Perk(2, "Woodsmen", "No movement penalty from *GREEN*forests*GREEN*."));
	perks.push_back(Perk(2, "Mountaineers", "No movement penalty from *GREEN*hills*GREEN*."));
	perks.push_back(Perk(2, "Advanced Logging", "No *GOLD*gold cost*GOLD* for building over forests."));
	perks.push_back(Perk(2, "Equine Husbandry", "Cavalry units *GREEN*+1*GREEN* Movement."));
