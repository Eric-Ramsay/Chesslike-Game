...
enum UNITS {
	PEASANT, SPEARMAN, AXEMAN, HEAVY_INFANTRY, ARCHER, SKIRMISHER, LIGHT_CAVALRY,
...
std::vector<UI_Unit> initUnits() { //Initialize unit types
	...
	units.push_back(UI_Unit(4, 6, 6, 4, 2, 3)); //6) skirmisher
	
	units.push_back(UI_Unit(6, 12, 5, 4, 1, 5)); //7) Light Cavalry
	units.push_back(UI_Unit(4, 20, 8, 4, 1, 8)); //8) Heavy Cavalry
	units.push_back(UI_Unit(5, 6, 4, 2, 3, 5)); //10) Cav Archer
	units.push_back(UI_Unit(4, 20, 8, 4, 1, 8)); //11) Commander
	units.push_back(UI_Unit(2, 12, 2, 2, 2, 8)); //12) Mayor
	units[10].can_buy = false;
	units[10].recruits = true;
	units[11].can_buy = false;
	units[11].recruits = true;
	
	units.push_back(UI_Unit(3, 6, 2, 2, 2, 2, true)); //13) fishing raft
	units.push_back(UI_Unit(4, 15, 3, 3, 3, 6, true)); //14) warship
	units.push_back(UI_Unit(5, 12, 8, 4, 1, 6, true)); //15) trireme
