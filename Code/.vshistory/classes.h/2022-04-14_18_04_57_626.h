...
enum POLICY_NAME {
	HOUSING, FISHING, LEVY, PIKES, RANGERS, HARDY, LOGGING, HUSBANDRY, HILLFORTS, MEDICINE, BARRACKS, VOLUNTEERS
};
std::vector<Policy> initPolicies() {
	...
	policies.push_back(Policy(2, "Caravans", "Merchants have *GREEN*+1*GREEN* MP and *GREEN*+2*GREEN* HP."));
	policies.push_back(Policy(2, "Fortifications", "You can *GREEN*recruit*GREEN* units on forts and piers"));
	policies.push_back(Policy(2, "Volunteers", "You can *GREEN*recruit*GREEN* units on forts and piers"));
...
std::vector<UI_Building> initBuildings() {
	...
	buildings.push_back(UI_Building(2, 13, 10)); //Stakes 1
	...
	buildings.push_back(UI_Building(5, 14, 15)); //Trade Depot 6
	buildings.push_back(UI_Building(6, 9, 15, false, true, 6, 8)); //Workshop 7
	buildings.push_back(UI_Building(8, 6, 15, false, false, 6, 6)); //Windmill 8
...
struct UI_Unit {
	...
	UI_Unit(short m, short h, short a, short d, short r, short c, bool n = false, bool cb = true) {
		sY = 0;
...
enum UNITS {
	...
	COMMANDER, MAYOR, FISHING_RAFT, WARSHIP, TRIREME, MAGE, MERCHANT, MANGONEL
};
...
std::vector<UI_Unit> initUnits() { //Initialize unit types
	...
	units.push_back(UI_Unit(3, 8, 5, 3, 1, 3)); //3) axeman
	...
	units.push_back(UI_Unit(4, 20, 8, 4, 1, 8, false, false)); //10) Commander
	units.push_back(UI_Unit(2, 12, 2, 2, 2, 8, false, false)); //11) Mayor

	units.push_back(UI_Unit(3, 6, 2, 2, 2, 2, true)); //12) fishing raft
	units.push_back(UI_Unit(4, 12, 3, 3, 3, 6, true)); //13) warship
	units.push_back(UI_Unit(5, 12, 8, 4, 1, 6, true)); //14) trireme
	
	units.push_back(UI_Unit(2, 8, 4, 4, 4, 8)); //14) mage

	units.push_back(UI_Unit(2, 12, 3, 3, 2, 4)); //14) merchant
	units.push_back(UI_Unit(2, 20, 8, 0, 4, 6)); //14) mangonel
	units.push_back(UI_Unit(2, 10, 4, 5, 1, 2, false, false)); //15) pikemen
