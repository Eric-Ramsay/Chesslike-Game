...
struct Perk {
	...
	Perk(std::string n, std::string desc) {
		name = n;
		description = desc;
	}
};

std::vector<Perk> initPerks() {
	std::vector<Perk> perks = {};
	
	perks.push_back(Perk("Efficient Housing", "*RED*Houses*RED* provide *GREEN*+3*GREEN* *GOLD*population*GOLD* space."));
	perks.push_back(Perk("Sustainable Fishing", "*ORANGE*Fish*ORANGE* replenish every turn."));
	perks.push_back(Perk("Forced Levy", "No upkeep for *PURPLE*workers*PURPLE* and *PURPLE*spearmen*PURPLE*."));
	perks.push_back(Perk("Sharper Spears", "*PURPLE*Spearmen*PURPLE* *GREEN*+1*GREEN* *BLUE*ATK*BLUE*."));
	perks.push_back(Perk("Woodsmen", "No *GREY*movement penalty*GREY* from *GREEN*forests*GREEN*."));
	perks.push_back(Perk("Mountaineers", "No *GREY*movement penalty*GREY* from *GREEN*hills*GREEN*."));
	perks.push_back(Perk("Advanced Logging", "No *GOLD*gold*GOLD* cost for building on *GREEN*forests*GREEN*."));
	perks.push_back(Perk("Equine Husbandry", "*PURPLE*Cavalry*PURPLE* units *GREEN*+1*GREEN* Movement."));
