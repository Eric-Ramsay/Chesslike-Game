...
std::vector<Policy> initPolicies() {
	...
	policies.push_back(Policy(2, "Efficient Housing", "Houses provide*GREEN* +3 *GREEN* population space."));			//coded
	policies.push_back(Policy(4, "Sustainable Fishing", "*ORANGE*Fish*ORANGE* replenish every turn."));					//coded
	policies.push_back(Policy(3, "Forced Levy", "Slain peasants don't give enemies *GOLD*gold*GOLD*. Workers/Spears *GREEN*no upkeep*GREEN*."));	//coded
	policies.push_back(Policy(2, "Sharper Spears", "Spearmen *GREEN*+1*GREEN* ATK."));									//coded
	policies.push_back(Policy(2, "Rangers", "No *GREEN*movement*GREEN* penalties from forests or elevation."));			//coded
	policies.push_back(Policy(1, "Hardy Folk", "Peasants and fishing ships have 8 Max HP and 2 ATK"));					//coded
	policies.push_back(Policy(2, "Advanced Logging", "No *GOLD*gold cost*GOLD* for building over forests."));			//coded
	policies.push_back(Policy(1, "Equine Husbandry", "Cavalry units *GREEN*+1*GREEN* Movement."));						//coded
	policies.push_back(Policy(1, "Hillforts", "Units with an elevation advantage have *GREEN*+1*GREEN* defense."));		//coded
	policies.push_back(Policy(2, "Field Medicine", "*GREEN*Heal*GREEN* one unit for 4 HP each turn."));					//coded
	policies.push_back(Policy(2, "Barracks", "Units on forts and piers pay *GREEN*no Upkeep*GREEN*"));					//coded
	policies.push_back(Policy(2, "Volunteers", "You may *GREEN*recruit*GREEN* units on forts and piers"));				//coded
