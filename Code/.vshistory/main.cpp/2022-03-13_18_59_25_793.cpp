		...
		if (drawMap && map.size() == MAPSIZE) {
			...
			if (makeCiv > -1) {
				...
				if (makeCiv == 0) {
					...
					if (hovered > -1) {
						Print(perks[hovered].name, ScreenWidth() / 2, dY + 123*scale, true);
						Print(perks[hovered].description, ScreenWidth() / 2, dY + 123 * scale, true);
						Print(perks[hovered].description, ScreenWidth() / 2, 2 * ScreenHeight() / 3 + 20 * i, true);
					}
				}
			}
			else {
