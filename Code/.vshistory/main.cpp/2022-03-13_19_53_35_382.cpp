			...
			if (makeCiv > -1) {
				...
				if (makeCiv == 0) {
					...
					for (int i = 0; i < perks.size(); i++) {
						...
						FillRectDecal(dX + 2 * scale + (i % 4) * 39 * scale, dY + 2 * scale + (i / 4) * 39 * scale, 36 * scale, 36 * scale, col);
					...
					if (hovered > -1) {
						Print(perks[hovered].name, ScreenWidth() / 2, dY + 120*scale, true, 2);
						Print(perks[hovered].description, ScreenWidth() / 2 - 12 * scale, dY + 138 * scale, true, 2, 300);
					}
				}
			}
			else {
