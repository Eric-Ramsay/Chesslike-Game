...
public:
						...
						if (turn == p.turn && map[tH.y][tH.x].unit.type > 0 && map[tS.y][tS.x].unit.type > 0) {
							...
							if (map[tS.y][tS.x].unit.MP > 0 && x_dist <= units[map[tS.y][tS.x].unit.type - 1].range && y_dist <= units[map[tS.y][tS.x].unit.type - 1].range) {
								if (map[tH.y][tH.x].unit.owner != p.turn && map[tS.y][tS.x].unit.owner == p.turn) {
									//Attack Unit!
									map[tH.y][tH.x].unit.HP -= getAttack(map[tS.y][tS.x].unit);
									//Unit Defends itself
		...
		if (drawMap && map.size() == MAPSIZE) {
			...
			if (makeCiv > -1) {
				...
				if (makeCiv == 1) { //Units
					...
					Print(to_str(getAttack(hovered)), dX + 53 * 2*scale, dY + 2 * 2*scale, false, 2 * scale); //ATK
	...
	//Stats
	int getAttack(int type, int owner) {
		int ape = units[a.type - 1].ATK;
		if (players[a.owner].policies[SPEARS]) {
			if (u.type == 2) {
				ape = ape + 1;
			}
		}
	...
	int getMaxMP(Unit a) {
		int ape = units[a.type - 1].MP;
		if (players[a.owner].policies[HUSBANDRY]) {
			if (u.type >= 5 && u.type <= 8) {
				return ape + 1;
			}
		}
		return ape;
	...
	int getMaxHP(Unit a) {
		int ape = units[a.type - 1].HP;
		if (players[a.owner].policies[HARDY]) {
			if (u.type == 1 || u.type == 11) {
				return 8;
			}
		}
		return ape;
