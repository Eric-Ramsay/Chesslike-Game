	...
	bool OnUserUpdate(float fElapsedTime) override {
		...
		if (drawMap && map.size() == MAPSIZE) {
			if (makeCiv) {
				...
				FillRectDecal(dX+scale, dY+scale, 162 * scale, 168 * scale, UI_Blue1);
				//Title Bar
				...
				for (int i = 0; i < perks.size(); i++) {
					//Draw(icons, 8.0 + 18*i*3.0, 8.0, 142, 142, 18, 18, 3.0);
				...
				Print(perks[0].name, ScreenWidth() / 2, ScreenHeight() / 3, true);
				Print(perks[0].description, ScreenWidth() / 2, 2 * ScreenHeight() / 3, true);
			}
			else {
				DrawMap();
				DrawDrag();
