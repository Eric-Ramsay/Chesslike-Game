				...
				if (dragging_unit) { //Move a Unit
					if (buying_unit || buying_building) {
						...
						if (turn == p.turn) {
							...
							float cost = moveCost(Spot(drag.x, drag.y), Spot(tH.x, tH.y), GetMaxMP(map[y][x].unit.type, map[y][x].unit.owner), map[drag.y][drag.x].unit);
						...
						if (turn == p.turn && map[tH.y][tH.x].unit.type > 0 && map[tS.y][tS.x].unit.type > 0) {
							...
							if (map[tS.y][tS.x].unit.MP > 0 && x_dist <= units[map[tS.y][tS.x].unit.type - 1].range && y_dist <= units[map[tS.y][tS.x].unit.type - 1].range) {
								if (map[tH.y][tH.x].unit.owner != p.turn && map[tS.y][tS.x].unit.owner == p.turn) {
									//Attack Unit!
									map[tH.y][tH.x].unit.HP -= getATK(map[tS.y][tS.x].unit);
									//Unit Defends itself
	...
	void DrawUI() {
		...
		if (MM_active) {
			...
			if (map[tH.y][tH.x].unit.type != 0) {
				...
				dY += 7 * scl;
				dX += 1;
				Print(to_str(map[tH.y][tH.x].unit.HP), dX+18*scl, dY, true, scl); //HP
				Print(to_str(map[tH.y][tH.x].unit.MP), dX+38*scl, dY, true, scl); //MP
				Print(to_str(GetATK(map[tH.y][tH.x].unit)), dX+55*scl, dY, true, scl); //ATK
