	...
	bool canBuyBuilding(int x, int y, int index) {
		...
		for (int i = -1; i < 2; i++) {
			for (int j = -1; j < 2; j++) {
				...
				else if (map[y][x].unit.type == 10) {
					if (units[map[safeC(y + i)][safeC(x + j)].unit.type == 1 || map[safeC(y + i)][safeC(x + j)].unit.type == 9) && map[safeC(y + i)][safeC(x + j)].unit.owner == p.turn) {
