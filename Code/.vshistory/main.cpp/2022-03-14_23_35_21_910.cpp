		...
		if (drawMap && map.size() == MAPSIZE) {
			...
			if (makeCiv > -1) {
				...
				if (makeCiv == 0) { //Units
					...
					//HP
					Print(to_str(u.HP), dX + 35 * scale, dY + 14 * scale, true, 2*scale);
					//ATK
					Print(to_str(u.ATK), dX + 53 * scale, dY + 2 * scale, false, 2*scale);
					//DEF
					Print(to_str(u.DEF), dX + 53 * scale, dY + 10 * scale, false, 2*scale);
					//RANGE
					Print(to_str(u.range), dX + 70 * scale, dY + 2 * scale, false, 2*scale);
					//MP
					Print(to_str(u.MP), dX + 70 * scale, dY + 10 * scale, false, 2*scale);
				}
