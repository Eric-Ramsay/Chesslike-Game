...
public:
					...
					if (buying_unit || buying_building) {
						...
						if (turn == p.turn) {
							...
							if (map[tH.y][tH.x].unit.type == 0 && cost <= map[drag.y][drag.x].unit.MP) {
								...
								if (map[tH.y][tH.x].unit.type == 10 || map[tH.y][tH.x].unit.type == 11) {
	...
	float tCost(int x, int y, int turn, int type) {
		Tile t = map[y][x];
		float price = 1.0;
		if (type == 10 || type == 11) { //boats
			if ((t.elev == WATER && t.building != 8)) {
				return 1;
			}
			return 999.0;
		}
