		...
		if (attacking_unit) {
			if (true || (map[tS.y][tS.x].unit.type > 0 && map[tS.y][tS.x].unit.owner == p.turn)) {
				...
				if (sFound) {
					...
					float dist_x = ((sX + 8*z) - hX) * ((sX + 8*z) - hX);
					float dist_y = ((sY + 8*z) - hY) * ((sY + 8*z) - hY);
					float dist = sqrt(dist_x + dist_y);
					if (dist < zoom) {
						dist = zoom;
					}

					DrawPartialRotatedDecal(olc::vf2d(sX + 8 * z, sY + 8 * z), icons, angle, olc::vf2d(6, 9), olc::vf2d(80, 80), olc::vf2d(12, 10), olc::vf2d(1.0, dist/zoom), colors[p.turn]);
				}
			}
		}
	}
