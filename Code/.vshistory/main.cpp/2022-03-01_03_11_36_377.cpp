...
public:
	...
	void DrawBuilding(int index, int dX, int dY, float zom, olc::Pixel pix) {
		float z = zom / 16.0;
		int index = 0;

		for (int i = 1; i < buildings.size(); i++) {
			if (buildings[i].type > map[y][x].building) {
				index = i - 1;
				i = buildings.size();
			}
		}
		...
		if (buildings[index].top_color) {
			DrawPartialDecal(olc::vf2d(dX, dY - zom), buildings, olc::vf2d(sX, sY - 16), olc::vf2d(16, 16), olc::vf2d(z, z), colors[map[y][x].owner], pix);
		}
		if (buildings[index].bot_color) {
			DrawPartialDecal(olc::vf2d(dX, dY), buildings, olc::vf2d(sX, 0), olc::vf2d(16, 16), olc::vf2d(z, z), colors[map[y][x].owner], pix);
		}
	}
