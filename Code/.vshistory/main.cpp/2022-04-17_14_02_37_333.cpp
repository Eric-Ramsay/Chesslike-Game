...
public:
	...
	//! Different from Unit functions as you pass index, not type !//
	int getBuildingHP(int index, int owner) {
	...
	void startTurn() {
		...
		for (int i = p.buildings.size() - 1; i >= 0; i--) {
			...
			if (map[c.y][c.x].owner == p.turn && map[c.y][c.x].building > 0) {
				bType = bIndex(map[c.y][c.x].building);
				if (map[c.y][c.x].HP >= getBuildingHP(bType, map[c.y][c.x].owner) / 2.0) {
				...
				if (map[c.y][c.x].HP < getBuildingHP(bType, map[c.y][c.x].owner)) {
					delta = true;
					map[c.y][c.x].HP++;
				...
				if (bType != PIER) { //check adjacent buildings for fire that might spread
					for (int a = -1; a < 2; a++) {
						for (int b = -1; b < 2; b++) {
							if (a != 0 || b != 0) {
								if (map[safeC(c.y + a)][safeC(c.x + b)].building > 0 && map[safeC(c.y + a)][safeC(c.x + b)].HP < getBuildingHP(bIndex(map[safeC(c.y + a)][safeC(c.x + b)].building), map[safeC(c.y + a)][safeC(c.x + b)].owner) / 2) {
									if (rand() % 2 == 0) {
										map[c.y][c.x].HP -= 2;
										a = 2;
										delta = true;
