...
public:
	...
	void startTurn() {
		...
		for (int i = p.buildings.size() - 1; i >= 0; i--) {
			...
			if (map[c.y][c.x].owner == p.turn) {
				...
				else if (map[c.y][c.x].building == 8) { //bridge
					p.max_buildings += 1;
				}
	...
	bool canBuyBuilding(int x, int y, int index) {
		...
		if (index == 2) { //bridge
			...
			bool t = map[safeC(tH.y - 1)][tH.x].elev >= FLAT || map[safeC(tH.y - 1)][tH.x].building == 8;
			bool d = map[safeC(tH.y + 1)][tH.x].elev >= FLAT || map[safeC(tH.y - 1)][tH.x].building == 8;
			bool l = map[tH.y][safeC(tH.x - 1)].elev >= FLAT || map[tH.y][safeC(tH.x - 1)].building == 8;
			bool r = map[tH.y][safeC(tH.x + 1)].elev >= FLAT || map[tH.y][safeC(tH.x + 1)].building == 8;
			bool td = ( || map[safeC(tH.y + 1)][tH.x].elev >= FLAT) && map[tH.y][safeC(tH.x - 1)].elev < FLAT && map[tH.y][safeC(tH.x + 1)].elev < FLAT;
