...
public:
	...
	bool canBuy(int x, int y, int index) {
		...
		if (!s) {
			for (int i = -1; i < 2; i++) {
				for (int j = -1; j < 2; j++) {
					if ((map[safeC(y + i)][safeC(x + j)].unit.type == 8 || map[safeC(y + i)][safeC(x + j)].unit.type == 9) && map[safeC(y + i)][safeC(x + j)].unit.owner == p.turn) {
						s = true;
						i = 2; j = 2;
					}
					else if (map[y][x].unit.type == 10 && units[map[safeC(y + i)][safeC(x + j)].unit.type-1].naval) {
						i = 2; j = 2;
					}
				}
			}
