...
public:
	...
	void DrawDrag() { //Draw unit that is being dragged
		...
		if (dragging_unit) {
			...
			DrawPartialDecal(olc::vf2d(dX, dY), armySprites, olc::vf2d(sX, sY + 16), olc::vf2d(16, 16), olc::vf2d(z, z), ap);
			...
			DrawPartialDecal(olc::vf2d(dX, dY - zoom), armySprites, olc::vf2d(units[unit_selected].sX, units[unit_selected].sY + 16), olc::vf2d(16, 17), olc::vf2d(z, z), ap);
		}
	...
	void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY, int zom = 16, bool borders = false, bool extra = true) {
		...
		if (zom >= 16) {
			...
			if (map[y][x].unit.type > 0 && !(dragging_unit && drag.x == x && drag.y == y)) { //Unit on Tile
				...
				DrawPartialDecal(olc::vf2d(dX, dY - (off*z)), armySprites, olc::vf2d(sX, sY+16), olc::vf2d(16, 16), olc::vf2d(z, z), ap);
