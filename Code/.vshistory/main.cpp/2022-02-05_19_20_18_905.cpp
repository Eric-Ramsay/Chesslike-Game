	...
	void startTurn() {
		...
		for (int i = p.buildings.size() - 1; i >= 0; i--) {
			...
			if (map[c.y][c.x].owner == p.turn) {
				...
				if (map[c.y][c.x].building == 5) { //House
					p.max_pop += 3;
				}
	...
	void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY, int zom = 16, bool borders = false, bool extra = true) {
		...
		if (zom >= 16) {
			...
			if (map[y][x].unit.type > 0 && !(dragging_unit && drag.x == x && drag.y == y)) { //Unit on Tile
				...
				sY = units[map[y][x].unit.type - 1].sY;
				//  x  y sX  sY  w   h  scale
				DrawPartialDecal(olc::vf2d(dX, dY - (4*z + zom)), armySprites, olc::vf2d(sX, sY), olc::vf2d(16, 32), olc::vf2d(z, z));
				DrawPartialDecal(olc::vf2d(dX, dY - (4 * z + zom)), armySprites, olc::vf2d(sX, sY+32), olc::vf2d(16, 32), olc::vf2d(z, z), ap);

				int hp = 12 * ((float)map[y][x].unit.HP / units[map[y][x].unit.type - 1].HP);
				int mov = 12 * ((float)map[y][x].unit.MP / units[map[y][x].unit.type - 1].MP);

				DrawPartialDecal(olc::vf2d(dX + 2 * z, (dY - (4 * z + zom)) + 10 * z), flat, olc::vf2d(418, 0), olc::vf2d(1, 1), olc::vf2d(hp*z, z));
				DrawPartialDecal(olc::vf2d(dX + 2 * z, (dY - (4 * z + zom)) + 12 * z), flat, olc::vf2d(420, 0), olc::vf2d(1, 1), olc::vf2d(mov*z, z));
			}
		...
		while (connected) {
			if (buffer.size() > 0) {
				if (timer == target) {
					...
					int byt = send(sock, s.c_str(), s.size() + 1, 0);
