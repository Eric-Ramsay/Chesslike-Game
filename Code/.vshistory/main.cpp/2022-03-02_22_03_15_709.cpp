	...
	void DrawUI() {
		...
		if (MM_active) {
			...
			if (unit_UI < 2) { //unit icon
				...
				Draw(icons, ScreenWidth() - (2 + 31 * scale) + 2 * scale, 2 + 2 * scale, 96, 16, 11, 11, scale, colors[p.turn]); //Player colors 2 34
			}
			else { //boat icon
				...
				Draw(icons, ScreenWidth() - (2 + 31 * scale) + 2 * scale, 2 + 2 * scale); //Player colors
			}
