...
public:
	...
	void DrawDrag() { //Draw unit that is being dragged
		...
		if (dragging_unit) {
			...
			DrawPartialDecal(olc::vf2d(dX, dY - zoom), armySprites, olc::vf2d(units[unit_selected].sX, sY + 32), olc::vf2d(16, 17), olc::vf2d(z, z), p);
		}
			...
			if (map[y][x].building == 0) {
				...
				if (building_selected == 0) { //farm
					...
					sX = 480;
					DrawPartialDecal(olc::vf2d(dX, dY - zoom), flat, olc::vf2d(sX, sY), olc::vf2d(16, 32), olc::vf2d(z, z), pix);
					DrawPartialDecal(olc::vf2d(dX, dY - zoom), flat, olc::vf2d(sX, sY - 32), olc::vf2d(16, 16), olc::vf2d(z, z), p);
				}
			}
		}
		if (attacking_unit) {
			if (map[tS.y][tS.x].unit.type > 0 && map[tS.y][tS.x].unit.owner == p.turn) {
				x = safeC(j + xView);
				y = safeC(i + yView);
				...
				bool sFound = false;
				bool hFound = false;
				if (tS.x >= x && tS.x <= x + w && tS.y >= y && tS.y <= y + h) {
					if (dXView > 0) { w += 2; }
					if (dYView > 0) { h += 2; }
					for (int i = -1 - ceil(h / 2); i <= 1 + ceil(h / 2); i++) {
						for (int j = -1 - ceil(w / 2); j <= ceil(w / 2); j++) {
							...
							int x = safeC(j + xView);
							if (y == tH.y && x == tH.x) {
								hX = (ScreenWidth() / 2 + ((j * zoom) - dXView));
								hY = (ScreenHeight() / 2 + ((i * zoom) - dYView));
								hFound = true;
							}
							else if (y == tH.y && x == tH.x) {
				...
				if (sFound && hFound) {
					float angle = -atan2((sX * 16 + 8) - (hX * 16 + 8), (sY * 16 + 8) - (hY * 16 + 8)) - .75;
					rDraw(icons, angle, sX + 8*z, sY + 8*z, 80, 80, 16, 16, z)
				}
