...
private:
	...
	//List of units, buildings, and perks
	...
	bool OnUserUpdate(float fElapsedTime) override {
		...
		if (drawMap && map.size() == MAPSIZE) {
			...
			if (makeCiv > -1) {
				...
				//Body
				FillRectDecal(dX + 2 * scale, dY + 11 * scale, 159 * scale, 118 * scale, UI_Blue4);
				//Footer
				FillRectDecal(dX + 2 * scale, dY + 129 * scale, 159 * scale, 38 * scale, UI_Blue3);
				if (makeCiv == 0) { //Units
					int num = 6;
					int num_drawn = 0;
					...
					for (int i = 1; i < units.size(); i++) { //Draw each unit in the body
						if (units[i].can_buy) {
							...
							if (range(GetMouseX(), GetMouseY(), dX + (num_drawn % 4) * 39 * scale, dY + (i / 4) * 39 * scale, 36 * scale, 36 * scale)) {
								hovered = i;
							...
							FillRectDecal(dX + 2 * scale + (num_drawn % 4) * 39 * scale, dY + 2 * scale + (num_drawn / 4) * 39 * scale, 32 * scale, 32 * scale, UI_Blue3);
							Draw(icons, dX + 2 * scale + (num_drawn % 4) * 39 * scale, dY + 2 * scale + (num_drawn / 4) * 39 * scale, perks[i].sX, perks[i].sY, 16, 16, 2 * scale);
						}
					}
					...
					if (points <= 0) {
						Print("*RED*" + to_str(points) + "*RED*", ScreenWidth() / 2 + 75 * scale, dY + scale + 6 * scale, true, 2);
					}
					else {
						Print("*GREEN*"+ to_str(points) + "*GREEN*", ScreenWidth() / 2 + 75 * scale, dY + scale + 6 * scale, true, 2);
					}
					//Footer Title
					FillRectDecal(dX + 2 * scale, dY + 129 * scale, 159 * scale, 9 * scale, UI_Blue2);
					Print("Perks", ScreenWidth() / 2, dY + scale + 6 * scale, true, 2);
					

					dX += 5 * scale;
					dY += 14 * scale;
					if (hovered >= perks.size()) {
						hovered = perks.size() - 1;
					}
					for (int i = 0; i < perks.size(); i++) {
						if (perks[i].active) {
							col = Gold;
						...
						FillRectDecal(dX + 2 * scale + (i % 4) * 39 * scale, dY + 2 * scale + (i / 4) * 39 * scale, 32 * scale, 32 * scale, UI_Blue3);
						Draw(icons, dX + 2*scale + (i % 4) * 39 * scale, dY + 2*scale + (i / 4) * 39 * scale, perks[i].sX, perks[i].sY, 16, 16, 2*scale);
						//Check mouse stuff in this loop!!
					}
					Print(perks[hovered].name + " - *GOLD*" + to_str(perks[hovered].cost) + "*GOLD*", ScreenWidth() / 2, dY + 120 * scale, true, 2);
					Print(perks[hovered].description, ScreenWidth() / 2, dY + 137 * scale, true, 2, 150 * scale);
				}
			}
			else {
				DrawUI();
