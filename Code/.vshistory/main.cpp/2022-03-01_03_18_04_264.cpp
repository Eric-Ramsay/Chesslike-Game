...
public:
	...
	void DrawBuilding(int type, int dX, int dY, float zom, olc::Pixel owner = olc::WHITE, olc::Pixel pix = olc::WHITE) {
		...
		if (index == 0) { //Farm
			...
			DrawPartialDecal(olc::vf2d(dX, dY - zom), buildings, olc::vf2d(sX, sY), olc::vf2d(16, 32), olc::vf2d(z, z), pix);
		}
		if (buildings[index].top_color) {
			DrawPartialDecal(olc::vf2d(dX, dY - zom), buildings, olc::vf2d(sX, sY - 16), olc::vf2d(16, 16), olc::vf2d(z, z), owner);
		}
		if (buildings[index].bot_color) {
			DrawPartialDecal(olc::vf2d(dX, dY), buildings, olc::vf2d(sX, 0), olc::vf2d(16, 16), olc::vf2d(z, z), owner);
		}
	}
	...
	void DrawDrag() { //Draw unit that is being dragged
		...
		if (dragging_unit) {
			...
			if (building_selected == 1) {
				DrawPartialDecal(olc::vf2d(dX, dY - zoom), buildings, olc::vf2d(sX, sY - 16), olc::vf2d(16, 16), olc::vf2d(z, z), colors[p.turn]);
			}
			else if (building_selected == 3) {
				DrawPartialDecal(olc::vf2d(dX, dY), buildings, olc::vf2d(sX, sY - 16), olc::vf2d(16, 16), olc::vf2d(z, z), colors[p.turn]);
			}
