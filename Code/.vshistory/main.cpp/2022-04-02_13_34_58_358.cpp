					...
					if (buying_unit || dragging_unit) {
						...
						if (turn == p.turn && canBuyBuilding(tH.x, tH.y, building_selected)) {
							...
							if (map[tH.y][tH.x].building == 8) { //Canals arent actually buildings
								...
								map[tH.y][tH.x].HP = buildings[building_selected].HP/2; //Give the building its HP
							}
	...
	void DrawUI() {
		...
		if (MM_active) {
			...
			if (map[tH.y][tH.x].building > 0) {
				...
				FillRectDecal(dX + 21*scl, dY + 4*scl + (29*2-hp), 5*scl, hp, colors[map[tH.y][tH.x].owner]);
			}
		}
		//Off 16 zoom warning
