		...
		if (drawMap && map.size() == MAPSIZE) {
			...
			if (makeCiv > -1) {
				...
				if (makeCiv == 0) { //Units
					...
					if (num <= 0) {
						Print("*RED*" + to_str(num) + "*RED*", ScreenWidth() / 2 + 75 * scale, dY + scale + 6 * scale, true, 2);
					}
					else {
						Print("*GREEN*" + to_str(num) + "*GREEN*", ScreenWidth() / 2 + 75 * scale, dY + scale + 6 * scale, true, 2);
					}
					...
					Print(to_str(u.cost), dX + 35 * scale, dY + 6 * scale, true, 2*scale); //Cost					
					Print(to_str(u.HP), dX + 35 * scale, dY + 14 * scale, true, 2*scale); //HP		
					Print(to_str(u.ATK), dX + 53 * scale, dY + 2 * scale, false, 2 * scale); //ATK
					Print(to_str(u.DEF), dX + 53 * scale, dY + 10 * scale, false, 2*scale); //DEF
					Print(to_str(u.range), dX + 70 * scale, dY + 2 * scale, false, 2*scale); //RANGE
					Print(to_str(u.MP), dX + 70 * scale, dY + 10 * scale, false, 2*scale); //MP
				}
