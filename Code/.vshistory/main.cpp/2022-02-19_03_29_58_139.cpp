...
private:
	...
	std::vector<const char *> music = { "./Sounds/bow_draw.mp3", "./Sounds/song1.mp3", "./Sounds/song2.mp3" };
	...
	void handleAudio() {
		...
		if (!audio_init) {
			...
			if (sound.atEnd || !started) {
				started = true;
				std::cout << "playing" << std::endl;
				ma_result res = ma_sound_init_from_file(&audio, music[music_index], 0, NULL, NULL, &sound);
				if (res != MA_SUCCESS) {
