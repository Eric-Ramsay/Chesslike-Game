...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override {
		...
		if (drawMap && map.size() == MAPSIZE) {
			...
			DrawDrag();
			DrawUI();
			Print(to_str(audio.inlinedSoundCount), ScreenWidth() / 2, 100, true, 3.0);
			Print(to_str(audio.pResourceManager->config.jobThreadCount), ScreenWidth() / 2, 170, true, 3.0);
		}
