...
public:
	...
	bool canBuy(int x, int y, int index) {
		...
		if (!s) {
			...
			return s && map[y][x].forest == NONE && map[y][x].elev != MOUNTAIN && map[y][x].unit.type == 0;
		}
	...
	bool canBuyBuilding(int x, int y, int index) {
		...
		for (int i = -1; i < 2; i++) {
			for (int j = -1; j < 2; j++) {
				if ((map[safeC(y + i)][safeC(x + j)].unit.type == PEASANT || map[safeC(y + i)][safeC(x + j)].unit.type == 9) && map[safeC(y + i)][safeC(x + j)].unit.owner == p.turn) {
					s = true;
					i = 2; j = 2;
				}
			}
