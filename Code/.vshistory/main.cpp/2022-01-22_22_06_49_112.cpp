	...
	void DrawDrag() { //Draw unit that is being dragged
		...
		if (attacking_unit) {
			if (true || (map[tS.y][tS.x].unit.type > 0 && map[tS.y][tS.x].unit.owner == p.turn)) {
				...
				if (sFound) {
					float angle = -atan2((sX * 16 + 8) - (hX * 16 + 8), (sY * 16 + 8) - (hY * 16 + 8));
					float dist_x = coord_dist(tS.x, tH.x);
					float dist_y = coord_dist(tS.y, tH.y);
					float dist = sqrt(dist_x * dist_x + dist_y * dist_y);
					DrawPartialRotatedDecal(olc::vf2d(sX + 8 * z, sY + 8 * z), icons, angle, olc::vf2d(8, 8), olc::vf2d(80, 80), olc::vf2d(16, 16), olc::vf2d(1.0, fDist(tS, hX, sY, hY)/z), colors[map[y][x].unit.owner]);
				}
			}
		}
	}
