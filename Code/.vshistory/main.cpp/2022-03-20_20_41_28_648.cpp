...
public:
				...
				if (dragging_heal) {
					...
					if (!GetMouse(0).bHeld) { //Heal unit
						if (turn == p.turn && map[tH.y][tH.x].unit.type > 0 && map[tH.y][tH.x].unit.owner == p.turn) {
							p[tH.y][tH.x].unit.HP = min(map[tH.y][tH.x].unit.HP + 4, GetMaxHP(map[tH.y][tH.x].unit.type, p.turn));
							can_heal = false;
						}
						dragging_heal = false;
					}
	...
	void DrawUI() {
		...
		if (MM_active) {
			...
			//Unit Heal
			Draw(icons, , (4 + (24+17) * scale), 48, 80, 16, 16, scale);
			if (!dragging_heal) {
				heal.x = ScreenWidth() - (2 + 17 * scale);
				heal.y = ScreenWidth() - (2 + 17 * scale)
