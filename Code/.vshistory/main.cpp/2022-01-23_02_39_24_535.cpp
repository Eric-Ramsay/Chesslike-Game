...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override {
		...
		if (!UI_Click) {
			...
			if (attacking_unit) {
				if (!GetMouse(1).bHeld) {
					float range = c_dist(tH, tS);
					if (turn == p.turn && range <= units[map[tS.y][tS.x].unit.type - 1].range) {
						if (map[tH.y][tH.x].unit.type > 0 && map[tS.y][tS.x].unit.type > 0) {
							if (map[tH.y][tH.x].unit.owner == p.turn && map[tS.y][tS.x].unit.owner != p.turn) {
								...
								if (range <= units[map[tH.y][tH.x].unit.type - 1].range) {
									map[tH.y][tH.x].unit.HP -= units[map[tS.y][tS.x].unit.type - 1].DEF;
								}
							}
						}
					}
	...
	void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY, int zom = 16, bool borders = false, bool extra = true) {
		...
		if (zom >= 16) {
			...
			if (tH.y == y && tH.x == x) {
				olc::Pixel pix = Gold;
				if (attacking_unit) {
					if (c_dist(tH, tS) <= units[map[tS.y][tS.x].unit.type - 1].range) {
						pix = olc::RED;
					}
				}
			...
			if (tS.y == y && tS.x == x) {
				DrawPartialDecal(olc::vf2d(dX, dY), flat, olc::vf2d(16, 16), olc::vf2d(16, 16), olc::vf2d(z, z), UI_LightPurple);
			}
		}
		else {
