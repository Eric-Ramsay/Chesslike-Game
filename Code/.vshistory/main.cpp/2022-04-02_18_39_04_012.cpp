...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override {
		...
		if (drawMap && makeCiv == -1) {
			...
			if (!UI_Click) {
				...
				if (attacking_unit) {
					if (!GetMouse(1).bHeld && turn == p.turn && map[tS.y][tS.x].unit.type > 0 && map[tS.y][tS.x].unit.MP > 0 && map[tS.y][tS.x].unit.owner == p.turn) {
						if (map[tH.y][tH.x].unit.type > 0 && map[tH.y][tH.x].unit.owner != p.turn) { //Attack an enemy unit
							//Get the distance to the hovered tile
							int x_dist = coord_dist(tH.x, tS.x);
							int y_dist = coord_dist(tH.y, tS.y);
							//If it is close enough, then the enemy is in range
							if (x_dist <= units[map[tS.y][tS.x].unit.type - 1].range && y_dist <= units[map[tS.y][tS.x].unit.type - 1].range) { //Make sure enemy is in range
						...
						else if (map[tS.y][tS.x].unit.type == PEASANT || map[tS.y][tS.x].unit.type == MAYOR || map[tS.y][tS.x].unit.type == FISHING_RAFT) { //peasant/mayor/fishing ship harvests fish
							...
							if (map[tS.y][tS.x].unit.MP > 0 && x_dist <= units[map[tS.y][tS.x].unit.type - 1].range && y_dist <= units[map[tS.y][tS.x].unit.type - 1].range) {
								...
								for (int i = -1; i < 2; i++) {
									for (int j = -1; j < 2; j++) {
										if ((i == 0 || j == 0) && i != j) { //check for adjacent enemy forts
								...
								if (bIndex(map[tH.y][tH.x].building) == FORT || proceed) { //dmg the building
									int dmg = 
								}
							}
						}
		...
		for (int i = p.units.size()-1; i >= 0; i--) {
			...
			if (map[c.y][c.x].unit.owner == p.turn && map[c.y][c.x].unit.type > 0) {
				...
				if (map[c.y][c.x].unit.type != 9) { //mayors dont pay upkeep costs
					if (!p.policies[LEVY] || (map[c.y][c.x].unit.type != PEASANT && map[c.y][c.x].unit.type != SPEARMAN && map[c.y][c.x].unit.type != FISHING_RAFT)) { //levy means no upkeep on fish, peasants, and spears
				...
				for (int a = -1; a < 2; a++) {
					for (int b = -1; b < 2; b++) {
						if (map[c.y][c.x].unit.type == MAGE) { //adjacent mage provides healing
							map[c.y][c.x].unit.HP += 2;
							a = 2;
							b = 2;
						}
	...
	void endTurn() {
		std::string s = "b"; //Claim Buildings, this is done at the end of the turn to prevent u from stepping on each building to claim it
		for (C c : p.units) {
	...
	bool canBuy(int x, int y, int index) {
		...
		if (units[index].naval) { //ships
			return s && map[y][x].elev < FLAT&& map[y][x].building != 8 && map[y][x].unit.type == NO_UNIT && p.gold >= units[index].cost;
		}
	...
	void DrawUI() {
		...
		if (MM_active) {
			...
			if (map[tH.y][tH.x].building > 0) {
				...
				DrawBuilding(map[tH.y][tH.x].building, dX + 3*scl, dY - 7 * scl, scl, C(tH.x, tH.y), colors[map[tH.y][tH.x].owner]);
				...
				int hp = 29*2 * ((float)map[tH.y][tH.x].HP / buildings[bIndex(map[tH.y][tH.x].building)].HP);
