...
public:
	...
	void DrawBuilding(int type, int dX, int dY, float zom, olc::Pixel owner = olc::WHITE, olc::Pixel pix = olc::WHITE) {
		float z = zom / 16.0;
		int index = 0;
		...
		if (index == 0) { //Farm
			...
			if (buildings[index].anim_num > 1) {
				sY += 32 * (((int)(randC(x, y) * 5) + timer / buildings[index].anim_speed) % buildings[index].anim_num);
			}
			DrawPartialDecal(olc::vf2d(dX, dY - zom), building_sprites, olc::vf2d(sX, sY), olc::vf2d(16, 32), olc::vf2d(z, z), pix);
		}
		if (buildings[index].top_color) {
			DrawPartialDecal(olc::vf2d(dX, dY - zom), building_sprites, olc::vf2d(sX, sY - 16), olc::vf2d(16, 16), olc::vf2d(z, z), owner);
		}
