...
s += '0' + t.unit.MP;
	...
	bool canBuyBuilding(int x, int y, int index) {
		...
		if (index == 2) { //bridge	
			bool t = map[safeC(tH.y - 1)][tH.x].elev >= FLAT || map[safeC(tH.y - 1)][tH.x].building == 12;
			bool d = map[safeC(tH.y + 1)][tH.x].elev >= FLAT || map[safeC(tH.y - 1)][tH.x].building == 12;
			bool l = map[tH.y][safeC(tH.x - 1)].elev >= FLAT || map[tH.y][safeC(tH.x - 1)].building == 13;
			bool r = map[tH.y][safeC(tH.x + 1)].elev >= FLAT || map[tH.y][safeC(tH.x + 1)].building == 13;
			bool td = (t || d) && !(l || r);
			bool lr = (l || r) && !(t || d);
			for (int i = -1; i < 2; i++) {
				for (int j = -1; j < 2; j++) {
					...
					if (i == 0 && td && map[safeC(y + i)][safeC(x + j)].building == 13) {
						return false;
					}
					if (j == 0 && lr && map[safeC(y + i)][safeC(x + j)].building == 12 || map[safeC(y + i)][safeC(x + j)].building == 12) {
						return false;
					}
				}
			}
