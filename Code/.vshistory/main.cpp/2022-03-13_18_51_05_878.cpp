		...
		if (drawMap && map.size() == MAPSIZE) {
			...
			if (makeCiv > -1) {
				...
				if (makeCiv == 0) {
					...
					//Footer Cost
					FillRectDecal(dX + 135 * scale, dY + 139 * scale, 25 * scale, 14 * scale, UI_Blue2);

					dX += 5 * scale;
					dY += 14 * scale;
					for (int i = 0; i < perks.size(); i++) {
						FillRectDecal(dX+(i%4)*39*scale, dY+(i/4)*39*scale, 36 * scale, 36 * scale, UI_Grey);
						Draw(icons, dX + 2*scale + (i % 4) * 39 * scale, dY + 2*scale + (i / 4) * 39 * scale, 144, 144, 16, 16, 2*scale);
						Draw(icons, dX + 2*scale + (i % 4) * 39 * scale, dY + 2*scale + (i / 4) * 39 * scale, 64, 144, 16, 16, 2*scale);
						//Check mouse stuff in this loop!!
						if (range(GetMouseX(), GetMouseY(), dX + (i % 4) * 39 * scale, dY + (i / 4) * 39 * scale))
						if (GetMouseX() >= dX + (i % 4) * 39 * scale && GetMouseY() >= dY + (i / 4) * 39 * scale) {
							if (GetMouseX() < = ) {

							}
						}
						//Print(perks[i].name, ScreenWidth() / 2, ScreenHeight() / 3 + 20 * i, true);
						//Print(perks[i].description, ScreenWidth() / 2, 2 * ScreenHeight() / 3 + 20 * i, true);
					}
				}
			}
			else {
