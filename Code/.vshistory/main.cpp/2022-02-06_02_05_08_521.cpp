...
public:
		...
		if (!UI_Click) {
			...
			if (buying_building) { //Buy a Building
				if (buying_unit || dragging_unit) {
					...
					if (turn == p.turn && canBuyBuilding(map, tH.x, tH.y, building_selected)) {
						p.buildings.push_back(tH);
						map[tH.y][tH.x].owner = p.turn;
			...
			if (buying_unit) { //Buy a Unit
				if (buying_building || dragging_unit) {
					...
					if (turn == p.turn && canBuy(map, tH.x, tH.y, unit_selected)) {
	...
	void DrawDrag() { //Draw unit that is being dragged
		...
		if (dragging_unit) {
			...
			dY += 4 * z;
			ap.a = 125;
			if (canBuy(map, tH.x, tH.y, unit_selected)) {
				pix = olc::WHITE;
				pix.a = 200;
				ap.a = 200;
			}
			else {
				bool ape = canBuy(map, tH.x, tH.y, unit_selected);
			}
			...
			x = tH.x;
			y = tH.y;
			if (canBuyBuilding(map, tH.x, tH.y, building_selected)) {
				pix = olc::WHITE;
				pix.a = 200;
				ap.a = 200;
			}
