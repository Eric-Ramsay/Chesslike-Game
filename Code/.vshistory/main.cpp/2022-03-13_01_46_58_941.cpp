	...
	bool OnUserUpdate(float fElapsedTime) override {
		...
		if (drawMap && map.size() == MAPSIZE) {
			if (makeCiv > -1) {
				...
				if (makeCiv == 0) { //Pick Units
					Print(perks[0].name, ScreenWidth() / 2, ScreenHeight() / 4, true);
					Print(perks[0].description, ScreenWidth() / 2, 3*ScreenHeight() / 4, true);
				}
				else if (makeCiv == 1) { //Pick Bonuses

				}
				else if (makeCiv == 2) { //Finalize
