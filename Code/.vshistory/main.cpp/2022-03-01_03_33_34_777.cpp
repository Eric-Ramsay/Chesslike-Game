...
private:
	...
	//List of units and list of units
...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override {
		...
		if (!MM_active) {
			...
			if (GetKey(olc::Key::U).bPressed) {
				...
				if (unit_UI == 1) {
					...
					unit_start = 9; unit_end = 11;
				}
				unit_bounds.h = 2 * 3.0 + (19 * 3.0 * ((1 + unit_end) - unit_start));
			}
		...
		if (drawMap) {
			...
			if (unit_UI > 0) {
				if (inBox(x, y, unit_bounds)) {
					UI_Click = true;
					if (GetMouse(0).bPressed) {
						unit_selected = min(unit_end, unit_start + (y - unit_bounds.y) / (unit_bounds.h / ((1 + unit_end) - unit_start)));
						buying_unit = true;
					}
				}
			}
			...
			if (!UI_Click) {
				...
				if (attacking_unit) {
					if (!GetMouse(1).bHeld) {
						...
						else if (turn == p.turn && (map[tS.y][tS.x].unit.type == 1 || map[tS.y][tS.x].unit.type == 9 || map[tS.y][tS.x].unit.type == 10)) { //peasant/mayor/fishing ship harvests fish
		...
		if (!drawMap) {
			...
			DrawPartialDecal(olc::vf2d(dX - 80, dY + 32), building_sprites, olc::vf2d(32, 16 + 32 * ((timer / 5) % 6)), olc::vf2d(16, 32), olc::vf2d(2, 2));
			DrawPartialDecal(olc::vf2d(dX - 16, dY + 32), building_sprites, olc::vf2d(32, 16 + 32 * ((1 + (timer / 5)) % 6)), olc::vf2d(16, 32), olc::vf2d(2, 2));
			DrawPartialDecal(olc::vf2d(dX + 48, dY + 32), building_sprites, olc::vf2d(32, 16 + 32 * ((2  + (timer / 5)) % 6)), olc::vf2d(16, 32), olc::vf2d(2, 2));
		}
	...
	bool canBuy(int x, int y, int index) {
		...
		if (units[index].naval) { //ships
			return s && p.units.size() < p.max_pop && map[y][x].elev < FLAT && map[y][x].building == 0 && map[y][x].unit.type == 0 && p.gold >= units[index].cost;
		}
	...
	void DrawUI() {
		...
		if (MM_active) {
			...
			if (unit_UI < 2) {
				...
				Draw(icons, ScreenWidth() - (2 + 31 * scale) + 2 * scale, 2 + 2 * scale, 96, 32, 15, 15, scale);
			}
			...
			if (unit_UI> 0) { //Unit Purchasing
				...
				int selected_dY = -1;
				for (int i = unit_start; i <= unit_end; i++) {
					Draw(icons, dX, dY, sX, sY, sW, sH, scale);
					UI_Unit u = units[i];
					Draw(armySprites, dX + 2 * scale, dY + scale, u.sX, 15 + u.sY, 16, 17, scale);
					//Print Cost
			...
			if (building_UI) { //building purchasing
				...
				for (int i = 0; i < buildings.size(); i++) {
					DrawBuilding(buildings[i].type, C(dX, dY), C(0, 0), scale * 16.0, colors[p.turn]);
					dX += 18 * scale;
	...
	void DrawBuilding(int type, int dX, int dY, float zom, C c = C(15, 15), olc::Pixel owner = olc::WHITE, olc::Pixel pix = olc::WHITE) {
		float z = zom / 16.0;
		int index = 0;
		...
		if (index == 0) { //Farm
			...
			DrawPartialDecal(olc::vf2d(d.x, d.y - zom), building_sprites, olc::vf2d(sX, sY), olc::vf2d(16, 32), olc::vf2d(z, z), pix);
		}
		if (buildings[index].top_color) {
			DrawPartialDecal(olc::vf2d(d.x, d.y - zom), building_sprites, olc::vf2d(sX, sY - 16), olc::vf2d(16, 16), olc::vf2d(z, z), owner);
		}
		if (buildings[index].bot_color) {
			DrawPartialDecal(olc::vf2d(d.x, d.y), building_sprites, olc::vf2d(sX, 0), olc::vf2d(16, 16), olc::vf2d(z, z), owner);
		}
	}
	...
	void DrawDrag() { //Draw unit that is being dragged
		...
		if (dragging_unit) {
			...
			DrawPartialDecal(olc::vf2d(dX, dY - zoom), building_sprites, olc::vf2d(sX, sY), olc::vf2d(16, 32), olc::vf2d(z, z), pix);
			if (building_selected == 1) {
				DrawPartialDecal(olc::vf2d(dX, dY - zoom), building_sprites, olc::vf2d(sX, sY - 16), olc::vf2d(16, 16), olc::vf2d(z, z), colors[p.turn]);
			}
			else if (building_selected == 3) {
				DrawPartialDecal(olc::vf2d(dX, dY), building_sprites, olc::vf2d(sX, sY - 16), olc::vf2d(16, 16), olc::vf2d(z, z), colors[p.turn]);
			}
			else if (building_selected == 5) {
				DrawPartialDecal(olc::vf2d(dX, dY), building_sprites, olc::vf2d(sX, sY - 16), olc::vf2d(16, 16), olc::vf2d(z, z), colors[p.turn]);
			}*/
			DrawBuilding(buildings[building_selected].type, dX, dY-zoom, colors[p.turn], pix);
		}
		if (attacking_unit) {
	...
	void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY, int zom = 16, bool borders = false, bool extra = true) {
		...
		if (zom >= 16) {
			...
			if (map[y][x].building > 0) {
				DrawBuilding(map[y][x].building, dX, dY, zom, colors[map[y][x].owner]);
			}
			if (map[y][x].forest > FISH) {
				sY = 0;
