...
public:
			...
			if (dragging_unit) { //Move a Unit
				if (buying_unit || buying_building) {
					...
					if (turn == p.turn) {
						...
						int cost = moveCost(Spot(drag.x, drag.y), Spot(tH.x, tH.y), units[map[drag.y][drag.x].unit.type - 1].MP, map[drag.y][drag.x].unit.owner);
	...
	float moveCost(Spot start, Spot end, int MP, int turn) {
		if (dist(start.x, start.y, end.x, end.y) > 2*MP) {
			return 99;
		}
		std::deque<Spot> open;
	...
	void run_turn() {
		...
		int building_v[4] = { 0, 0, 0, 0 };
		int unit_v[8] = { 0, 0, 0, 0, 0, 0, 0, 0 }; //Relative strength of other players
		int unit_priority[7] = { 0, 0, 0, 0, 0, 0, 0 }; //Priority for purchasing units
		int my_val = 0;
		...
		for (int i = 0; i < MAPSIZE; i++) {
			for (int j = 0; j < MAPSIZE; j++) {
				if (map[i][j].unit.owner > 0) {
					unit_v[map[i][j].unit.owner - 1] += units[map[i][j].unit.type - 1].cost;
					if (map[i][j].unit.owner != p.turn) {
						switch (map[i][j].unit.type) {
						case 1: unit_priority[0] += 5; build_farm += 5; build_windmill += 5; break; //Peasant
						case 2: unit_priority[2] += 5; unit_priority[3] += 5; unit_priority[4] += 5; break; //Spearmen
						case 3: unit_priority[2] += 5; unit_priority[5] += 5; break; //Archers
		...
		if (!p.started) { //Place the commander somewhere
			...
			while (!p.started && runs < 100) {
				stop = false;
				C c = can_place[rand() % can_place.size()];
				for (int a = -1; a < 2; a++) {
					for (int b = -1; b < 2; b++) {
			...
			for (int i = 0; i < 7; i++) { //Give priority to unit purchasing based on what AI can buy
				if (p.gold >= units[i].cost) {
					unit_priority[i] += 15 + i * 5;
				}
				else {
					unit_priority[i] = 0;
				}
			}
			my_val = unit_v[p.turn - 1];
			int max = 0;
			for (int i = 0; i < 8; i++) {
				if (i != (p.turn - 1)) {
					if (unit_v[i] > max) {
			...
			if (has_commander) {
				...
				while (p.gold > 0 && !stop) {
					runs++;
					//Decide what to do
					if (p.max_pop > p.units.size()) {
						for (int i = 6; i >= 0; i--) { //Buy Units
					...
					index = -1;
					bIndex = -1;
					if (p.gold >= 3 && rand() % 100 < build_house) {
						int ape = 0;
						do {
							index = rand() % can_placeB.size();
						} while (can_placeB[index].x == -1 && ape++ < can_place.size());
					...
					if (p.gold >= 1 && rand() % 100 < build_farm) {
						int ape = 0;
						do {
							index = rand() % can_placeB.size();
						} while (can_placeB[index].x == -1 && ape++ < can_place.size());
					}
					if (p.gold >= 8 && rand() % 100 < build_windmill) { //Buy a windmill
						int ape = 0;
						do {
							index = rand() % can_placeB.size();
						} while (can_placeB[index].x == -1 && ape++ < can_place.size());
