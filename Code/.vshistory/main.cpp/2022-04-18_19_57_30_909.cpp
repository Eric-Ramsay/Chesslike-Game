		...
		if (drawMap && map.size() == MAPSIZE) {
			...
			if (makeCiv > -1) {
				...
				if (GetMouse(0).bPressed) {
					...
					dX + w - (29 * scale), dY + h - scale, 64, 112, 29, 11, scale);
					else if (points <= 1 && num == 0 && range(GetMouseX(), GetMouseY(), dX + w - (29 * scale), dY + 168 * scale, 29 * scale, 11 * scale)) { //done button clicked
						makeCiv = -1;
						int nextAdd = -1;
						bool add = false;
