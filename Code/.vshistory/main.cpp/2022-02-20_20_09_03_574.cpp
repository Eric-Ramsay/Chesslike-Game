		...
		if (MM_active) {
			...
			if (draw_encyclopedia) {
				...
				for (Entry e : encyclopedia) {
					...
					Print(to_str((int)std::roundf(e.income)), dX + 17 * scale, dY + 3 * scale , true, scale);
					Print(to_str(e.unit_value), dX + 33 * scale, dY + 3 * scale , true, scale);
					dY += 8 * scale;
				}
				//Draw the bottom
