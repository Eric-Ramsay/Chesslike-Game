...
private:
	...
	enum SFX {
		BOW_DRAW, BOW_FIRE, KILL, ATTACK, BUILD, UNIT_PLACE, CHIMES, FISH_SPLASH, MARCH, FOREST_BUILD, SHIP_MOVE, FIREBALL, CLINK
	};
...
public:
			...
			if (!UI_Click) {
				...
				if (buying_building) { //Buy a Building
					if (buying_unit || dragging_unit) {
						...
						if (turn == p.turn && canBuyBuilding(tH.x, tH.y, building_selected)) { //place building down
							p.buildings.push_back(tH);
							map[tH.y][tH.x].owner = p.turn;
							map[tH.y][tH.x].building = buildings[building_selected].type;
							if (map[tH.y][tH.x].building == 12) { //bridge
								bool l = map[tH.y][safeC(tH.x - 1)].elev >= FLAT || map[tH.y][safeC(tH.x - 1)].building == 13;
								bool r = map[tH.y][safeC(tH.x + 1)].elev >= FLAT || map[tH.y][safeC(tH.x + 1)].building == 13;
								if (l || r) {
									map[tH.y][tH.x].building = 13;
								}
							}
							...
							if (map[tS.y][tS.x].unit.MP > 0 && x_dist <= units[map[tS.y][tS.x].unit.type - 1].range && y_dist <= units[map[tS.y][tS.x].unit.type - 1].range) {
								if (map[tH.y][tH.x].unit.owner != p.turn && map[tS.y][tS.x].unit.owner == p.turn) {
									...
									if (map[tH.y][tH.x].unit.HP <= 0) { //Enemy unit dies
										if (map[tH.y][tH.x].unit.type == 11) { //Earn a gold if the unit is a fishing ship
											p.gold++;
										}
										else if (map[tH.y][tH.x].unit.type == 1) { //if the unit is a peasant, earn a gold if the player doesnt have forced levy
							...
							if (map[tH.y][tH.x].forest == FISH && map[tH.y][tH.x].harvest < harvest) {
								...
								if (map[tS.y][tS.x].unit.MP > 0 && x_dist <= units[map[tS.y][tS.x].unit.type - 1].range && y_dist <= units[map[tS.y][tS.x].unit.type - 1].range) {
									if (map[tS.y][tS.x].unit.owner == p.turn) {
										...
										if (p.policies[FISHING]) {
											map[tH.y][tH.x].harvest = harvest;
										}
										map[tS.y][tS.x].unit.MP = 0;
										sendData("u" + sendUnit(tH.x, tH.y) + sendUnit(tS.x, tS.y));
						...
						if (turn == p.turn && map[tH.y][tH.x].unit.type > 0 && map[tH.y][tH.x].unit.type != 8 && map[tH.y][tH.x].unit.owner == p.turn) {
							if (!units[map[tH.y][tH.x].unit.type - 1].naval) {
								if (p.gold >= (16 - units[map[tH.y][tH.x].unit.type - 1].cost)) {
									...
									map[tH.y][tH.x].unit.HP = min(map[tH.y][tH.x].unit.HP + 10, getMaxHP(8, p.turn));
									map[tH.y][tH.x].unit.MP = 0;
									sendData("u" + sendUnit(tH.x, tH.y));
								}
							}
				...
				if (dragging_heal) {
					...
					if (!GetMouse(0).bHeld) { //Heal unit
						if (turn == p.turn && map[tH.y][tH.x].unit.type > 0 && map[tH.y][tH.x].unit.owner == p.turn) {
							playSound(CLINK);
							map[tH.y][tH.x].unit.HP = min(map[tH.y][tH.x].unit.HP + 4, getMaxHP(map[tH.y][tH.x].unit.type, p.turn));
							sendData("u" + sendUnit(tH.x, tH.y));
							can_heal = false;
						}
						dragging_heal = false;
				...
				if (GetMouse(1).bPressed) {
					...
					if (turn == p.turn && map[tS.y][tS.x].unit.type > 0 && map[tS.y][tS.x].unit.MP > 0) {
						attacking_unit = true;
						if (units[map[tS.y][tS.x].unit.type - 1].range > 1 && map[tS.y][tS.x].unit.type != 15) {
							playSound(BOW_DRAW);
						}
					}
				}
		...
		if (drawMap && map.size() == MAPSIZE) {
			...
			if (makeCiv > -1) {
				...
				if (makeCiv == 1) { //Units
					...
					Print(to_str(getMaxHP(hovered+1, p.turn)), dX + 36 * 2*scale - 1, dY + 14 * 2*scale, true, 2*scale); //HP		
					Print(to_str(getATK(hovered+1, p.turn)), dX + 53 * 2*scale, dY + 2 * 2*scale, false, 2 * scale); //ATK
					...
					Print(to_str(getMaxMP(hovered+1, p.turn)), dX + 70 * 2*scale, dY + 10 * 2*scale, false, 2*scale); //MP
				}
	...
	int getATK(int type, int owner) {
		...
		if (players[owner].policies[SPEARS]) {
			if (type == 2) {
				ape = ape + 1;
			}
		}
	...
	void startTurn() {
		...
		for (int i = p.units.size()-1; i >= 0; i--) {
			...
			if (map[c.y][c.x].unit.owner == p.turn && map[c.y][c.x].unit.type > 0) {
				...
				if (map[c.y][c.x].unit.type != 9) { //mayors dont pay upkeep costs
					if (!p.policies[LEVY] || (map[c.y][c.x].unit.type != 1 && map[c.y][c.x].unit.type != 2 && map[c.y][c.x].unit.type != 11)) { //levy means no upkeep on fish, peasants, and spears
						if (!p.policies[BARRACKS] || (map[c.y][c.x].building != 7 && map[c.y][c.x].building != 11)) { //barracks means forts and piers provide upkeep for unit on that tile
				...
				else if (map[c.y][c.x].building == 7) { //Heal on Fort
					map[c.y][c.x].unit.HP += 5;
				}
				for (int a = -1; a < 2; a++) {
					for (int b = -1; b < 2; b++) {
						if (map[c.y][c.x].unit.type == 15) { //adjacent mage provides healing
							map[c.y][c.x].unit.HP += 2;
							a = 2;
							b = 2;
						}
		...
		for (int i = p.buildings.size() - 1; i >= 0; i--) {
			...
			if (map[c.y][c.x].owner == p.turn) {
				...
				else if (map[c.y][c.x].building == 12 || map[c.y][c.x].building == 13) { //bridge
					p.max_buildings += 1;
				}
				else if (map[c.y][c.x].building == 10) { //office
					p.max_buildings += 9;
				}
...
s += '0' + t.unit.MP;
	...
	bool canBuy(int x, int y, int index) {
		bool s = (!p.started);
		if (map[y][x].building == 7 && map[y][x].owner != p.turn) { //can't build units on enemy forts
			return false;
		}
		if (!s) {
			if (p.policies[VOLUNTEERS] && map[y][x].owner == p.turn && (map[y][x].building == 7 || map[y][x].building == 11)) {
				s = true;
			}
			else {
				for (int i = -1; i < 2; i++) {
		...
		if (units[index].naval) { //ships
			return s && map[y][x].elev < FLAT&& map[y][x].building != 8 && map[y][x].unit.type == 0 && p.gold >= units[index].cost;
		}
		return s && map[y][x].elev >= FLAT && map[y][x].forest == NONE && map[y][x].elev != MOUNTAIN && map[y][x].unit.type == 0 && p.gold >= units[index].cost;
	}
	...
	bool canBuyBuilding(int x, int y, int index) {
		...
		if (index == 2) { //bridge
			bool bdt = map[safeC(tH.y - 1)][tH.x].building == 12 || map[safeC(tH.y + 1)][tH.x].building == 12;
			bool blr = map[tH.y][safeC(tH.x - 1)].building == 13 || map[tH.y][safeC(tH.x + 1)].building == 13;
			bool ldt = map[tH.y][safeC(tH.x - 1)].elev >= FLAT || 
			bool t = map[safeC(tH.y - 1)][tH.x].elev >= FLAT || ;
			bool d = map[safeC(tH.y + 1)][tH.x].elev >= FLAT || ;
			bool l =  || ;
			bool r = map[tH.y][safeC(tH.x + 1)].elev >= FLAT || ;
			bool td = (t || d);
			bool lr = (l || r);
			if (td || lr) {
				for (int i = -1; i < 2; i++) {
					for (int j = -1; j < 2; j++) {
						if (map[safeC(y + i)][safeC(x + j)].building == 12 || map[safeC(y + i)][safeC(x + j)].building == 13) {
							if ((td && i == 0) || (lr && j == 0)) {
								return false;
							}
						}
	...
	void DrawUI() {
		...
		if (MM_active) {
			...
			Print(to_str(p.upkeep), 8 + 157 * scale, 2 + 8 * scale, true, scale);
	...
	void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY, int zom = 16, bool borders = false, bool extra = true) {
		...
		if (zom >= 16) {
			...
			if (map[y][x].unit.type > 0 && !(dragging_unit && drag.x == x && drag.y == y)) { //Unit on Tile
				...
				sY = units[map[y][x].unit.type - 1].sY;
				float off = 1;
				if (map[y][x].building == 7 || map[y][x].building == 12 || map[y][x].building == 13) { //Fort or Bridge
					off = 6;
				}
			...
			if (bytesReceived > 1) {
				...
				if (p.turn == 0) {
					...
					if (c == 'r') { //Get Row
						...
						for (int i = 1; i < min(NUM_POLICIES, msg.length()); i++) {
