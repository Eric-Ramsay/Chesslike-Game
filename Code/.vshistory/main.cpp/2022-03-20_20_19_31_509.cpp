	...
	void DrawUI() {
		...
		if (MM_active) {
			...
			if (map[tH.y][tH.x].unit.type != 0) {
				...
				dY += 7 * scl;
				dX += 1;
				//map[tH.y][tH.x].unit.HP
				Print(to_str(3), dX+18*scl, dY, true, scl); //HP
				Print((int)to_str(map[tH.y][tH.x].unit.MP), dX + 38 * scl, dY, true, scl); //MP
				Print((int)to_str(getATK(map[tH.y][tH.x].unit.type, map[tH.y][tH.x].unit.owner)), dX+55*scl, dY, true, scl); //ATK
