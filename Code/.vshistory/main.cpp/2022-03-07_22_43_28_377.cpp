...
private:
	std::vector<olc::Pixel> colors = { WHITE, olc::Pixel(16, 138, 231), olc::Pixel(166, 43, 58), olc::Pixel(39, 213, 216), olc::Pixel(76, 182, 92), olc::Pixel(231, 228, 0),  olc::Pixel(255, 140, 0) , olc::Pixel(114, 71, 255) , olc::Pixel(220, 0, 213) };
...
public:
		...
		if (MM_active) {
			...
			if (unit_UI> 0) { //Unit Purchasing
				...
				for (int i = unit_start; i <= unit_end; i++) {
					...
					Draw(armySprites, dX + 2 * scale, dY + 2*scale, u.sX, 17 + u.sY, 16, 16, scale, colors[p.turn]); //Draw Unit's colors
					//Print Cost
			...
			if (map[tH.y][tH.x].unit.type != 0) {
				int scl = 2;
				dX = ScreenWidth() - (4 + 112 * 2);
				dY = ScreenHeight() - (2 + 13 * 2);
				Draw(icons, dX, dY, 0, 179, 96, 13, 2);
				Print(to_str((int)map[tH.y][tH.x].unit.HP), dX+19*2, dY, true, 2); //HP
				Print(to_str((int)map[tH.y][tH.x].unit.MP), dX, dY, true, 2); //MP
				Print(to_str(units[map[tH.y][tH.x].unit.type-1].ATK), dX, dY, true, 2); //ATK
				Print(to_str(units[map[tH.y][tH.x].unit.type-1].DEF), dX, dY, true, 2); //DEF
				Print(to_str(units[map[tH.y][tH.x].unit.type-1].range), dX, dY, true, 2); //RANGE
			}
		}
		//Off 16 zoom warning
	...
	void DrawDrag() { //Draw unit that is being dragged
		...
		if (dragging_unit) {
			...
			//  x  y sX  sY  w   h  scale
			DrawPartialDecal(olc::vf2d(dX, dY), armySprites, olc::vf2d(sX, sY + 17), olc::vf2d(16, 16), olc::vf2d(z, z), ap); //Draw Color
			...
			Draw(armySprites, dX, dY - (zoom-z), units[unit_selected].sX, 17 + units[unit_selected].sY, 16, 16, z, colors[p.turn]);
		}
	...
	void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY, int zom = 16, bool borders = false, bool extra = true) {
		...
		if (zom >= 16) {
			...
			if (map[y][x].unit.type > 0 && !(dragging_unit && drag.x == x && drag.y == y)) { //Unit on Tile
				...
				//  x  y sX  sY  w   h  scale
				DrawPartialDecal(olc::vf2d(dX, dY - ((off-1)*z)), armySprites, olc::vf2d(sX, sY+17), olc::vf2d(16, 16), olc::vf2d(z, z), ap);
