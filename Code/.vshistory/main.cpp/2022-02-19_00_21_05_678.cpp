...
private:
	...
	void handleAudio() {
		...
		if (!audio_init) {
			...
			//ma_sound_group_init(&music_volume);
		}
		else if (drawMap && !started) { //Play music
			started = true;
			ma_fence fence;
			ma_fence_init(&fence);
			ma_sound sounds[2];
			for (int i = 0; i < music.size(); i++) {
				ma_sound_init_from_file(&audio, (const char *)music[i], MA_SOUND_FLAG_ASYNC, NULL, &fence, &sounds[i]);
			}
		}
	}
