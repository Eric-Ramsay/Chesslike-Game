		...
		if (drawMap && map.size() == MAPSIZE) {
			...
			if (makeCiv > -1) {
				...
				if (makeCiv == 1) { //Units
					...
					Print("Policies", ScreenWidth() / 2, dY + 6 * scale, true, 2);
					...
					dX -= 5 * scale;
					dY -= 14 * scale;
					Print(policies[hovered].name + " - *GOLD*" + to_str(policies[hovered].cost) + "*GOLD*", ScreenWidth() / 2, dY + h - (46 * scale), true, 2);
					Print(policies[hovered].description, ScreenWidth() / 2, dY + h - (46 * scale), true, 2, 150 * scale);
				}
			}
			else {
				DrawUI();
