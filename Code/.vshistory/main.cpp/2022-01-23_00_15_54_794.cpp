		...
		if (attacking_unit) {
			if (true || (map[tS.y][tS.x].unit.type > 0 && map[tS.y][tS.x].unit.owner == p.turn)) {
				...
				if (sFound) {
					...
					float dist_x = (sX - (hX - 8 * z)) * (sX - hX);
					float dist_y = (sY - hY) * (sY - hY);
					float dist = sqrt(dist_x + dist_y);
					if (dist < zoom) {
						dist = zoom;
					}
					Print(to_str(angle*180/3.1415), ScreenWidth() / 2, 30, 2);
					Print(to_str(dist), ScreenWidth() / 2, ScreenHeight()-30, 2);

					DrawPartialRotatedDecal(olc::vf2d(sX + 8 * z, sY + 8 * z), icons, angle, olc::vf2d(6, 9), olc::vf2d(80, 80), olc::vf2d(12, 10), olc::vf2d(z, dist/16.0), colors[p.turn]);
				}
			}
		}
	}
