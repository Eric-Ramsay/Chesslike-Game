...
private:
	...
	enum SFX {
		BOW_DRAW, BOW_FIRE, KILL, ATTACK, BUILD, UNIT_PLACE, CHIMES, FISH_SPLASH, MARCH, FOREST_BUILD
	};
...
public:
			...
			if (!UI_Click) {
				...
				if (attacking_unit) {
					if (!GetMouse(1).bHeld) {
						...
						else if (turn == p.turn && (map[tS.y][tS.x].unit.type == 1 || map[tS.y][tS.x].unit.type == 9)) { //peasant/mayor harvests fish
	...
	void DrawUI() {
		...
		if (MM_active) {
			...
			if (unit_UI == 1) { //Land Unit Purchasing
				dX = 2;
				dY = 4 + 15 * scale;

				//Calc # of units to draw: 
				//Draw Top
				sX = 0; sY = 48; sW = 76; sH = 1;
				Draw(icons, dX, dY, sX, sY, sW, sH, scale);
				dY += scale;
				//Draw Middle
				sX = 0; sY = 49; sW = 76; sH = 19;
				for (int i = unit_start; i <= unit_end; i++) {
					Draw(icons, dX, dY, sX, sY, sW, sH, scale);
					UI_Unit u = units[i];
					Draw(armySprites, dX + 2 * scale, dY + scale, u.sX, 15 + u.sY, 16, 17, scale);
					//Print Cost
					Print(to_str(u.cost), dX + 36 * scale, dY + 6 * scale, true, scale);
					//HP
					Print(to_str(u.HP), dX + 36 * scale, dY + 14 * scale, true, scale);
					//ATK
					Print(to_str(u.ATK), dX + 52 * scale, dY + 2 * scale, false, scale);
					//DEF
					Print(to_str(u.DEF), dX + 52 * scale, dY + 10 * scale, false, scale);
					//RANGE
					Print(to_str(u.range), dX + 68 * scale, dY + 2 * scale, false, scale);
					//MP
					Print(to_str(u.MP), dX + 68 * scale, dY + 10 * scale, false, scale);
					if (i == unit_selected) {
						DrawBox(dX + scale, dY + scale, 73 * scale, 17 * scale, Gold, scale);
					}
					dY += 18 * scale;
				}
				//Draw Bottom
			...
			else if (unit_UI == 2) { //Naval Unit Purchasing
