	...
	bool OnUserUpdate(float fElapsedTime) override {
		...
		if (!UI_Click) {
			...
			if (dragging_crown) {
				crown.x = x - 5 * 3.0;
				crown.y = y;
				if (!GetMouse(1).bHeld) { //Promote Unit to Commander
		...
		Print(to_str(commander_bounds.x), ScreenWidth() / 2, ScreenHeight() / 2 + 50, 3);
		Print(to_str(commander_bounds.y), ScreenWidth() / 2, ScreenHeight() / 2 + 75, 3);
