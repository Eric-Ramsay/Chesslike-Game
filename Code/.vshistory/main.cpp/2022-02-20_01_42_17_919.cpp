...
private:
	...
	//Game Loop Variables
	float fTargetFrameTime = 1.0f / 5.0; // Virtual FPS of 60fps
...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override {
			...
			if (!UI_Click) {
				...
				if (attacking_unit) {
					if (!GetMouse(1).bHeld) {
						...
						else if (turn == p.turn && (map[tS.y][tS.x].unit.type == 1 || map[tS.y][tS.x].unit.type == 9)) { //peasant/mayor
		...
		Print("dXView: " + to_str(dXView), ScreenWidth()/2, 200, true, 2.0);
