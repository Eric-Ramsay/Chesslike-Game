...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override {
		...
		if (!drawMap) {
			...
			DrawBuilding(6, dX-80, dY+32, 2, 99, C(10, 73));
			DrawBuilding(9, dX-16, dY+32, 2, 99, C(31, 5), colors[p.turn]);
			DrawBuilding(6, dX+48, dY+32, 2, 99, C(105, 73));
		}
	...
	void startTurn() {
		...
		for (int i = p.buildings.size() - 1; i >= 0; i--) {
			...
			if (map[c.y][c.x].owner == p.turn && map[c.y][c.x].building > 0) {
				bType = bIndex(map[c.y][c.x].building);
				if (map[c.y][c.x].HP >= buildings[bType].HP / 2.0) {
				...
				if (map[c.y][c.x].HP < buildings[bType].HP) {
					delta = true;
					map[c.y][c.x].HP++;
				...
				if (bType != PIER) { //check adjacent buildings for fire that might spread
					for (int a = -1; a < 2; a++) {
						for (int b = -1; b < 2; b++) {
							if (a != 0 || b != 0) {
								if (map[safeC(c.y + a)][safeC(c.x + b)].building > 0 && map[safeC(c.y + a)][safeC(c.x + b)].HP < buildings[bIndex(map[safeC(c.y + a)][safeC(c.x + b)].building)].HP / 2) {
									if (rand() % 2 == 0) {
										map[c.y][c.x].HP -= 2;
										a = 2;
										delta = true;
				...
				for (int i = -1; i < 2; i++) {
					for (int j = -1; j < 2; j++) {
						if (map[safeC(y + i)][safeC(x + j)].unit.owner == p.turn) {
							...
							else if (index == AXEMAN && units[map[safeC(y + i)][safeC(x + j)].unit.type - 1].naval) { //raiders can be spawned next to ships
		...
		if (MM_active) {
			...
			if (building_UI) { //building purchasing
				...
				for (int i = 0; i < buildings.size(); i++) {
					Draw(building_sprites, dX, dY, sX, sY, sW, sH, scale);
					if (i == 0) {
						DrawBuilding(4, dX+scale, dY - 8 * scale, scale, 99, C(0, 0), colors[p.turn]);
					}
					else {
						DrawBuilding(buildings[i].type, dX+scale, dY - 8 * scale, scale, 99, C(0, 0), colors[p.turn]);
					}
			...
			if (map[tH.y][tH.x].building > 0) {
				...
				DrawBuilding(map[tH.y][tH.x].building, dX + 3*scl, dY - 7 * scl, scl, map[tH.y][tH.x].HP, C(tH.x, tH.y), colors[map[tH.y][tH.x].owner]);
	...
	void DrawBuilding(int type, int dX, int dY, float scale, int HP = 99, C c = C(15, 15), olc::Pixel owner = olc::WHITE, olc::Pixel pix = olc::WHITE) {
		float z = scale;
		...
		if (HP < buildings[index].HP/2.0) { //draw fire sprite
			sX = 48;
			sY = 48;
	...
	void DrawDrag() { //Draw unit that is being dragged
		...
		if (dragging_unit) {
			...
			DrawBuilding(buildings[building_selected].type, dX, dY-zoom, z, 99, C(15, 15), colors[p.turn], pix);
		}
		if (attacking_unit) {
	...
	void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY, int zom = 16, bool borders = false, bool extra = true) {
		...
		if (zom >= 16) {
			...
			if (map[y][x].building > 0) {
				DrawBuilding(map[y][x].building, dX, dY-zom, z, map[y][x].HP, C(x, y), colors[map[y][x].owner]);
			}
			if (map[y][x].forest > FISH) {
				sY = 0;
