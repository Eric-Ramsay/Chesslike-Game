...
public:
		...
		if (drawMap && map.size() == MAPSIZE) {
			...
			if (makeCiv > -1) {
				...
				if (GetMouse(0).bPressed) {
					...
					else if (points <= 1 && num == 0 && range(GetMouseX(), GetMouseY(), dX + 134 * scale, dY + 168 * scale, 29 * scale, 11 * scale)) { //done button clicked
						makeCiv = -1;
						int nextAdd = -1;
						bool add = false;
				...
				if (makeCiv == 1) { //Units
					...
					dX = (ScreenWidth() - (w - 58 * scale)) / 2;
					dY = (ScreenHeight() - (h - 23 * scale)) / 2;
					Draw(icons, dX, dY, 19, 50, 58, 17, scale * 2);
					UI_Unit u = units[hovered];
					Print(to_str(u.cost), dX + 36 * 2*scale - 1, dY + 5 * 2*scale, true, 2*scale); //Cost					
					Print(to_str(getMaxHP(hovered+1, p.turn)), dX + 36 * 2*scale - 1, dY + 13 * 2*scale, true, 2*scale); //HP		
					Print(to_str(getATK(hovered+1, p.turn)), dX + 53 * 2*scale, dY + 1 * 2*scale, false, 2 * scale); //ATK
					Print(to_str(u.DEF), dX + 53 * 2*scale, dY + 9 * 2*scale, false, 2*scale); //DEF
					Print(to_str(u.range), dX + 70 * 2*scale, dY + 1 * 2*scale, false, 2*scale); //RANGE
					Print(to_str(getMaxMP(hovered+1, p.turn)), dX + 70 * 2*scale, dY + 9 * 2*scale, false, 2*scale); //MP
				}
	...
	int getRange(int type, int owner) {
		int ape = units[type - 1].range;
		if (players[owner].policies[PIKES]) {
			if (type == PIKEMAN) {
				return 2;
			}
		}
