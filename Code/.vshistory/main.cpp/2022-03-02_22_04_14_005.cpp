...
public:
	...
	void DrawUI() {
		...
		if (MM_active) {
			...
			if (unit_UI < 2) { //unit icon
				...
				Draw(icons, ScreenWidth() - (2 + 31 * scale) + 2 * scale, 2 + 2 * scale, 2, 34, 11, 11, scale, colors[p.turn+1]); //Player colors 2 34
			}
			else { //boat icon
				Draw(icons, ScreenWidth() - (2 + 31 * scale), 2, 96, 0, 15, 15, scale);
				Draw(icons, ScreenWidth() - (2 + 31 * scale) + 2 * scale, 2 + 2 * scale, 96, 32, 11, 1511 scale);
				Draw(icons, ScreenWidth() - (2 + 31 * scale) + 7 * scale, 2 + 5 * scale, 6, 38, 2, 1, scale, colors[p.turn+1]); //Player colors
			}
			
			//Player Turn and Current Turn
			Draw(icons, ScreenWidth()-(2+16*2), ScreenHeight()-(2+16*2), 0, 80, 16, 16, 2, colors[turn]);
			if (turn == p.turn) {
				Draw(icons, ScreenWidth() - (2 + 16 * 2), ScreenHeight() - (2 + 16 * 2), 16, 80, 16, 16, 2);
			}
			...
			if (unit_UI> 0) { //Unit Purchasing
				...
				for (int i = unit_start; i <= unit_end; i++) {
					...
					Draw(armySprites, dX + 2 * scale, dY + 2*scale, u.sX, 32 + u.sY, 16, 16, scale, colors[p.turn]);
					//Print Cost
	...
	void DrawDrag() { //Draw unit that is being dragged
		...
		if (dragging_unit) {
			...
			DrawPartialDecal(olc::vf2d(dX, dY), armySprites, olc::vf2d(sX, sY + 32), olc::vf2d(16, 16), olc::vf2d(z, z), ap); //Draw Color
			...
			Draw(armySprites, dX, dY - (zoom-z), units[unit_selected].sX, 32 + units[unit_selected].sY, 16, 16, z, colors[p.turn]);
		}
	...
	void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY, int zom = 16, bool borders = false, bool extra = true) {
		...
		if (zom >= 16) {
			...
			if (map[y][x].unit.type > 0 && !(dragging_unit && drag.x == x && drag.y == y)) { //Unit on Tile
				...
				DrawPartialDecal(olc::vf2d(dX, dY - (off*z)), armySprites, olc::vf2d(sX, sY+32), olc::vf2d(16, 16), olc::vf2d(z, z), ap);
