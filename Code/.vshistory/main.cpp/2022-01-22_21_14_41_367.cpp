...
public:
	...
	void DrawDrag() { //Draw unit that is being dragged
		...
		if (dragging_unit) {
			...
			DrawPartialDecal(olc::vf2d(dX, dY - zoom), armySprites, olc::vf2d(units[unit_selected].sX, sY + 32), olc::vf2d(16, 17), olc::vf2d(z, z), ap);
		}
		...
		if (attacking_unit) {
			if (true || (map[tS.y][tS.x].unit.type > 0 && map[tS.y][tS.x].unit.owner == p.turn)) {
				...
				if (c_less(x, tS.x) && c_less(tS.x, x + w/2) && c_less(y, tS.y) && c_less(tS.y, y + h/2)) {
					if (dXView > 0) { w += 2; }
					if (dYView > 0) { h += 2; }
