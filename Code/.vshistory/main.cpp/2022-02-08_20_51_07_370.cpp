	...
	void startTurn() {
		...
		for (int i = p.buildings.size() - 1; i >= 0; i--) {
			...
			if (map[c.y][c.x].owner == p.turn) {
				...
				else if (map[c.y][c.x].building == 9) { //House
					p.gold++;
				}
	...
	void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY, int zom = 16, bool borders = false, bool extra = true) {
		...
		if (zom >= 16) {
			...
			if (map[y][x].unit.type > 0 && !(dragging_unit && drag.x == x && drag.y == y)) { //Unit on Tile
				...
				//  x  y sX  sY  w   h  scale
				DrawPartialDecal(olc::vf2d(dX, dY - (offset + zom)), armySprites, olc::vf2d(sX, sY), olc::vf2d(16, 32), olc::vf2d(z, z));
				DrawPartialDecal(olc::vf2d(dX, dY - (offset + zom)), armySprites, olc::vf2d(sX, sY+32), olc::vf2d(16, 32), olc::vf2d(z, z), ap);
