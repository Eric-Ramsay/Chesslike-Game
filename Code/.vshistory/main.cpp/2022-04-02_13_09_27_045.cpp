...
public:
			...
			if (!UI_Click) {
					...
					if (buying_unit || dragging_unit) {
						...
						if (turn == p.turn && canBuyBuilding(tH.x, tH.y, building_selected)) {
							...
							if (map[tH.y][tH.x].building == 8) { //Canals arent actually buildings
								...
								map[tH.y][tH.x].building = buildings[building_selected].HP; //Give the building its HP
							}
				...
				if (attacking_unit) {
					if (!GetMouse(1).bHeld) {
						...
						else if (turn == p.turn && (map[tS.y][tS.x].unit.type == PEASANT || map[tS.y][tS.x].unit.type == MAYOR || map[tS.y][tS.x].unit.type == FISHING_RAFT)) { //peasant/mayor/fishing ship harvests fish
	...
	int bIndex(char t) {
		...
		for (int i = 0; i < buildings.size(); i++) {
			if (type >= buildings[i].min_type && type <= buildings[i].max_type) {
				return i;
			}
		}
	...
	void DrawUI() {
		...
		if (MM_active) {
			...
			if (map[tH.y][tH.x].unit.type != NO_UNIT) {
				...
				dX = ScreenWidth() - (4 + 135 * scl);
				Draw(icons, dX, dY, 0, 179, 119, 13, scl);
				dY += 7 * scl;
				dX += 1;
			...
			if (map[tH.y][tH.x].building > 0) {
				...
				DrawBuilding(map[tH.y][tH.x].building, dX + 3*scl, dY - 8 * scale, scl, C(tH.x, tH.y), colors[map[tH.y][tH.x].owner]);
				Print(to_str(buildings[map[tH.y][tH.x].building-1].loot), dX + 16*scl, dY + 29*scl, true, scl);
				Print(to_str((int)map[tH.y][tH.x].HP), dX + 19 * scl, dY + 37 * scl, true, scl);
				//FillRectDecal();
			}
		}
		//Off 16 zoom warning
