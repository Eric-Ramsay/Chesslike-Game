	...
	void startTurn() {
		...
		for (int i = p.buildings.size() - 1; i >= 0; i--) {
			...
			if (map[c.y][c.x].owner == p.turn && map[c.y][c.x].building > 0) {
				...
				if (bIndex == HOUSE || bIndex == FORT || bIndex == PIER) {
					int max_space = 1;
					if (bIndex == HOUSE || bIndex == FORT) {
						max_space = 3;
					}
				...
				else if (bIndex == OFFICE) { //office
					p.max_buildings += 9;
				}
				else if (bIndex == WORKSHOP) { //workshop
					p.gold++;
					p.max_pop -= 2;
				}
				...
				if (map[c.y][c.x].HP < buildings[bInd].HP) {
