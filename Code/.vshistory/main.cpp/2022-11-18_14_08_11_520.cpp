...
public:
				...
				if (dragging_unit) { //Move a Unit
					if (buying_unit || buying_building) {
						...
						if (turn == P_TURN && map[drag.y][drag.x].unit.owner == P_TURN) {
							...
							float cost = moveCost(Spot(drag.x, drag.y), Spot(tH.x, tH.y), getMaxMP(map[drag.y][drag.x].unit.type, players[map[drag.y][drag.x].unit.owner]), map[drag.y][drag.x].unit);
		...
		if (drawMap && map.size() == MAPSIZE) {
			...
			if (makeCiv > -1) {
				...
				if (makeCiv == 1) { //Units
					...
					Print(to_str(getMaxMP(hovered+1, players[P_TURN])), dX + 51 * 2*scale, dY + 9 * 2*scale, false, 2*scale); //MP
				}
	...
	float tCost(int x, int y, int turn, int type) {
		...
		if (bIndex(t.building) == STAKES) {
			return getMaxMP(type, players[turn]);
		}
