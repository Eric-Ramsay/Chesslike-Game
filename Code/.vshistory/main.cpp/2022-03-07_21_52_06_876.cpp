	...
	void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY, int zom = 16, bool borders = false, bool extra = true) {
		...
		if (zom >= 16) {
			...
			if (map[y][x].unit.type > 0 && !(dragging_unit && drag.x == x && drag.y == y)) { //Unit on Tile
				...
				//Draw a unit
				DrawPartialDecal(olc::vf2d(dX, dY - ((1+off)*z)), armySprites, olc::vf2d(sX, sY), olc::vf2d(16, 17), olc::vf2d(z, z));
				//  x  y sX  sY  w   h  scale
				DrawPartialDecal(olc::vf2d(dX, dY - (off*z)), armySprites, olc::vf2d(sX, sY+17), olc::vf2d(16, 16), olc::vf2d(z, z), ap);
