...
public:
			...
			if (clean) {
				for (int i = 0; i < map.size(); i++) {
					for (int j = 0; j < map.size(); j++) {
						...
						if (map[i][j].building > 0) {
							if (map[i][j].owner == p.turn) {
	...
	void fillType(std::vector<std::vector<Tile>>& map, int x, int y, Tile fill, int cut = 100) {
		Tile t = map[y][x];
	...
	void saveMap(std::vector<std::vector<Tile>>& map) {
		ofstream myfile;
		myfile.open("saves/save" + to_str(rand()) + ".txt");
		std::string s = "";
		for (int i = 0; i < map.size(); i++) {
			for (Tile t : map[i]) {
				s += '0' + t.type;
	...
	float tCost(int x, int y) {
		...
		for (int i = -1; i < 2; i++) {
			for (int j = -1; j < 2; j++) {
				if (i != 0 || j != 0) {
					if (map[safeC(i + y)][safeC(x + j)].unit.type > 0) {
						if (map[safeC(i + y)][safeC(x + j)].unit.owner != p.turn) {
							std::cout << (int)map[safeC(i + y)][safeC(x + j)].unit.owner << std::endl;
							std::cout << (int)map[safeC(i + y)][safeC(x + j)].unit.type << std::endl;
							price += .5;
						}
					}
				}
			...
			for (int a = -1; a < 2; a++) {
				for (int b = -1; b < 2; b++) {
					if ((a == 0 || b == 0) && a != b) { //only check 4 adjacent tiles
						...
						consider.g = current.g + tCost(consider.y, consider.x	); //Distance to start
						if (consider.g <= MP) {
