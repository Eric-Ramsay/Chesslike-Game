...
private:
	...
	//AUDIO FILES - I should make this a vector indexed by an enum
	SoLoud::Wav wav_attack;
	SoLoud::Wav wav_bow_draw;
	SoLoud::Wav wav_bow_fire;
	SoLoud::Wav wav_kill;
	SoLoud::Wav wav_build;
	SoLoud::Wav wav_chimes;
	SoLoud::Wav wav_unit_place;

	std::vector<SoLoud::Wav> music_list = {};
	...
	void initAudio() {
		...
		audio.init();
		music_list.resize(4);
		music_list[0].load("./Sounds/song1.wav");
		music_list[1].load("./Sounds/song2.wav");

		wav_bow_draw.load("./Sounds/bow_draw.wav");
		wav_bow_fire.load("./Sounds/bow_fire.wav");
		wav_kill.load("./Sounds/kill.wav");
		wav_attack.load("./Sounds/attack.wav");
		wav_build.load("./Sounds/build.wav");
		wav_unit_place.load("./Sounds/unit_place.wav");
		wav_chimes.load("./Sounds/chimes.wav");
	}
	void handleAudio() {
		if (!audio_init) {
			initAudio();
		...
		if (drawMap) {
			if (audio.getActiveVoiceCount() > 0) {
				return;
			}
			music_index = (music_index + 1) % music_list.size();
			audio.play(music_list[music_index]);
		}
	}
	//-------------------
	...
	//Game Loop Variables
	float fTargetFrameTime = 1.0f / 50.0; // Virtual FPS of 30fps
	...
	Box building_bounds = Box(2, 6 + 144 * 3, 71 * 3, 34 * 3);
...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override {
		...
		if (!GetKey(olc::Key::SHIFT).bHeld) {
			...
			if (GetKey(olc::Key::UP).bHeld || GetKey(olc::Key::W).bHeld) { //Pan Up
				...
				if (zoom >= (ScreenWidth()/MAPSIZE - 1) && zoom >= 2) {
					zoom--;
				}
			}
		...
		if (building_UI) {
			if (inBox(x, y, building_bounds)) {
				UI_Click = true;
				if (GetMouse(0).bPressed) {
					building_selected = min(3, (x - building_bounds.x) / (building_bounds.w / 4));
			...
			if (buying_building) { //Buy a Building
				if (buying_unit || dragging_unit) {
					...
					if (turn == p.turn && canBuyBuilding(tH.x, tH.y, building_selected)) {
						...
						else if (building_selected == 2) { //Windmill
							map[tH.y][tH.x].building = 6;
							p.gold -= 8;
						}
						else if (building_selected == 3) { //Fort
							map[tH.y][tH.x].building = 7;
							p.gold -= 8;
						}
						audio.play(wav_build);
						sendData("b" + sendBuilding(tH.x, tH.y));
					}
					buying_building = false;
				}
	...
	bool canBuyBuilding(int x, int y, int index) {
		...
		if (index == 2) { //bridge
			cost = 6;
			bool td = map[safeC(tH.y - 1)][tH.x].elev >= FLAT && map[safeC(tH.y + 1)][tH.x].elev >= FLAT;
			bool lr = map[tH.y][safeC(tH.x - 1)].elev >= FLAT && map[tH.y][safeC(tH.x + 1)].elev >= FLAT && map[safeC(tH.y - 1)][tH.x].elev < FLAT;
			s = s && (map[safeC(tH.y - 1)][tH.x].elev >= FLAT && map[safeC(tH.y + 1)][tH.x].elev >= FLAT)
		}
		s = s && (map[y][x].building == 0 || (GetKey(olc::Key::SHIFT).bHeld && map[y][x].owner == p.turn));
		return s && ((index != 2 && map[y][x].elev >= FLAT) || (index == 2 && map[y][x].elev < FLAT)) && map[y][x].forest == NONE && map[y][x].elev != MOUNTAIN && p.gold >= cost;
	}
	...
	float tCost(int x, int y, int turn) {
		Tile t = map[y][x];
		float price = 1.0;
		if (t.elev == WATER || t.unit.owner != turn && t.unit.type > 0) {
			price = 999.0;
		}
		else if (t.elev == HILL) {
			price = 1.5;
				...
				for (int b = -1; b < 2; b++) {
					if ((a != 0 || b != 0)) { 
						...
						if (consider.g <= MP) {
							...
							if (map[consider.y][consider.x].p_index != ind) {
								open.push_back(consider);
								map[consider.y][consider.x].p_index = ind;
							}
						}
					}
				}
	...
	void DrawUI() {
		...
		if (MM_active) {
			...
			if (building_UI) { //building purchasing
				dX = 2;
				dY = 6 + 144 * scale;
				Draw(buildings, dX, dY, 89, 174, 71, 34, scale);
	...
	void DrawDrag() { //Draw unit that is being dragged
		...
		if (dragging_unit) {
			...
			else if (building_selected == 3) { //Windmill
				sX = 32;
				sY += 32 * ((timer / 5) % 6);
			}
			else if (building_selected == 4) { //Fort
				sX = 48;
			}
	...
	void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY, int zom = 16, bool borders = false, bool extra = true) {
		...
		if (!buying_unit && !buying_building && map[tS.y][tS.x].unit.type > 0) {
			if (map[tS.y][tS.x].unit.owner == p.turn) {
				...
				if (map[tS.y][tS.x].unit.MP > 0 && moveCost(Spot(tS.x, tS.y), Spot(x, y), units[map[tS.y][tS.x].unit.type - 1].MP, map[tS.y][tS.x].unit.owner) <= map[tS.y][tS.x].unit.MP) {
					drawDot = true;
				}
			}
			else if (moveCost(Spot(tS.x, tS.y), Spot(x, y), units[map[tS.y][tS.x].unit.type - 1].MP, map[tS.y][tS.x].unit.owner) <= units[map[tS.y][tS.x].unit.type - 1].MP) {
				drawDot = true;
			}
		}
		dotColor.a = 125;
		...
		if (zom >= 16) {
			...
			if (map[y][x].building > 0) {
				sY = 16;
				if (map[y][x].building >= 1 && map[y][x].building <= 4) {
					sX = 0;
			...
			if (map[y][x].unit.type > 0 && !(dragging_unit && drag.x == x && drag.y == y)) { //Unit on Tile
				...
				sY = units[map[y][x].unit.type - 1].sY;
				//  x  y sX  sY  w   h  scale
				DrawPartialDecal(olc::vf2d(dX, dY - (5*z + zom)), armySprites, olc::vf2d(sX, sY), olc::vf2d(16, 32), olc::vf2d(z, z));
				DrawPartialDecal(olc::vf2d(dX, dY - (5 * z + zom)), armySprites, olc::vf2d(sX, sY+32), olc::vf2d(16, 32), olc::vf2d(z, z), ap);

				int hp = 12 * ((float)map[y][x].unit.HP / units[map[y][x].unit.type - 1].HP);
				int mov = 12 * ((float)map[y][x].unit.MP / units[map[y][x].unit.type - 1].MP);

				DrawPartialDecal(olc::vf2d(dX + 2 * z, (dY - zom) + 5 * z), flat, olc::vf2d(418, 0), olc::vf2d(1, 1), olc::vf2d(hp*z, z));
				DrawPartialDecal(olc::vf2d(dX + 2 * z, (dY - zom) + 7 * z), flat, olc::vf2d(420, 0), olc::vf2d(1, 1), olc::vf2d(mov*z, z));
			}
