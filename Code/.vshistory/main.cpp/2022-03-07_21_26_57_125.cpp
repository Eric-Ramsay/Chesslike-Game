		...
		if (MM_active) {
			...
			if (unit_UI> 0) { //Unit Purchasing
				...
				for (int i = unit_start; i <= unit_end; i++) {
					Draw(icons, dX, dY, sX, sY, sW, sH, scale);
					UI_Unit u = units[i];
					Draw(armySprites, dX + 2 * scale, dY + scale, u.sX, 15 + u.sY, 16, 17, scale); //Draw Unit
					Draw(armySprites, dX + 2 * scale, dY + 2*scale, u.sX, 32 + u.sY, 16, 16, scale, colors[p.turn]); //Draw Unit's colors
					//Print Cost
