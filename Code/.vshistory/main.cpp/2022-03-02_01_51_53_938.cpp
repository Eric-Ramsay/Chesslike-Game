...
public:
	...
	void DrawUI() {
		...
		if (MM_active) {
			...
			Draw(icons, ScreenWidth()-(2+16*scale), ScreenHeight()-(18+16*scale), 0, 80, 16, 16, scale, colors[turn]);
			if (turn == p.turn) {
				Draw(icons, ScreenWidth() - (2 + 16 * scale), ScreenHeight() - (18 + 16 * scale), 16, 80, 16, 16, scale);
			}
