			...
			for (int j = 0; j < MAPSIZE; j++) {
				if (map[i][j].unit.owner > 0) {
					...
					if (map[i][j].unit.owner != p.turn) {
						...
						case 1: unit_p[0] += 5; build_farm += 5; build_windmill += 5; break; //Peasant
		...
		if (!p.started) { //Place the commander somewhere
			...
			if (has_commander) {
				...
				while (p.gold > 0 && !stop) {
					runs++;
					//Decide what to do
					if (p.max_pop > p.units.size()) {//Buy Units
						for (int i = 6; i >= 0; i--) { 
					...
					for (int i = 0; i < 4; i++) {
						if (p.gold >= b_costs[i] && rand() % 100 < building_p[i]) {
							...
							if (ape < can_placeB.size()) {
								if (i == farm || i == fort) { //Fort/Farm can be random
									bIndex = i;
								}
								else { //Windmill or house
									if (i == windmill) {
