...
public:
	...
	void DrawUI() {
		...
		if (MM_active) {
			...
			Draw(icons, ScreenWidth()-(2+16*scale), ScreenHeight()-(2+16*scale), 0, 80, 16, 16, scale, colors[turn]);
			if (turn == p.turn) {
				Draw(icons, ScreenWidth() - (2 + 16 * scale), ScreenHeight() - (2 + 16 * scale), 16, 80, 16, 16, scale);
			}
		...
		if (zoom > 16 && zoom % 16 != 0) {
			Draw(icons, ScreenWidth() - 2, ScreenHeight() - (16*scale+4), 112, 176, 16, 16);
		}
		//FPS warning
		if (GetFPS() < 58) {
			Draw(icons, ScreenWidth() - 2, ScreenHeight() - (16 * scale + 22), 96, 176, 16, 16);
		}
	}
