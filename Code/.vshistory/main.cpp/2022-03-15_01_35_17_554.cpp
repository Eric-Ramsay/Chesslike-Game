	...
	bool OnUserUpdate(float fElapsedTime) override {
		...
		if (drawMap && map.size() == MAPSIZE) {
			...
			if (makeCiv > -1) {
				...
				if (makeCiv == 1) { //Units
					Draw(icons, dX + 17 * scale, dY - 12 * scale, 98, 7, 15, 13, scale);
					Draw(icons, dX, dY - 12 * scale, 80, 7, 15, 13, scale);
				...
				else if (makeCiv == 0) { //Perks
					Draw(icons, dX + 17 * scale, dY - 12 * scale, 80, 0, 15, 13, scale);
					Draw(icons, dX, dY - 12 * scale, 98, 7, 15, 13, scale);
