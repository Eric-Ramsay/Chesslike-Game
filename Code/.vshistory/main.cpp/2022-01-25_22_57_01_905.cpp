...
//SOUND VARIABLES---------------------------------------
//SoLoud::Soloud audio_settings; // SoLoud engine
//SoLoud::Wav background;      // One wave file
//SoLoud::Wav sfx;      // One wave file
//------------------------------------------------------
...
public:
	...
	bool OnUserCreate() override { //Runs once when game starts
		...
		buildings_s = new olc::Sprite("./Sprites/icons.png"); buildings = new olc::Decal(buildings_s);
	...
	void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY, int zom = 16, bool borders = false, bool extra = true) {
		...
		if (zom >= 16) {
			...
			if (map[y][x].building > 0) {
				...
				if (map[y][x].building == 5) { //Draw the flag on the house
					DrawPartialDecal(olc::vf2d(dX, dY - zom), flat, olc::vf2d(sX, sY - 16), olc::vf2d(16, 16), olc::vf2d(z, z), colors[map[y][x].owner]);
				}
			}
