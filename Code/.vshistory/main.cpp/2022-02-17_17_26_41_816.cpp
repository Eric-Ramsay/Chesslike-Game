			...
			for (int a = -1; a < 2; a++) {
				for (int b = -1; b < 2; b++) {
					if ((a != 0 || b != 0)) { 
						...
						if (consider.g <= MP) { 							// Loop through open and closed to see if the tile has already been checked.
	...
	void DrawMap() {
		...
		for (int i = -1 - ceil(height / 2); i <= 1 + ceil(height / 2); i++) {
			for (int j = -1 - ceil(width / 2); j <= ceil(width / 2); j++) {
				...
				if (map[tS.y][tS.x].unit.type > 0) {
					//
					float cost = moveCost(tS, Spot(x, y), units[map[tS.y][tS.x].unit.type - 1].MP, map[tS.y][tS.x].unit.owner);
					if (cost < 99) {
						Print(to_str(cost), (ScreenWidth() / 2 + ((j * zoom) - dXView)) + zoom / 2, (ScreenHeight() / 2 + ((i * zoom) - dYView)) + zoom / 2, true, zoom / 16.0);
					}
				}
			}
		}
