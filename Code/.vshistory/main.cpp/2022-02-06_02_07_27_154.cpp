	...
	void run_turn(std::vector<std::vector<Tile>>& map, Player& p) {
		...
		if (!p.started) { //Place the commander somewhere
			...
			while (!p.started && runs < 100) {
				stop = false;
				C spot = can_place[rand() % can_place.size()];
				for (int a = -1; a < 2; a++) {
					for (int b = -1; b < 2; b++) {
				...
				if (!stop) {
					p.started = true;
					p.units.push_back(spot);
					map[spot.y][spot.x].owner = p.turn;
					map[spot.y][spot.x].unit.type = unit_selected + 1;
					map[spot.y][spot.x].unit.owner = p.turn;
					map[spot.y][spot.x].unit.HP = units[unit_selected].HP;
					map[spot.y][spot.x].unit.MP = 0;
					sendData(sendUnit(tH.x, tH.y));
					run_turn(map); //Run this function again
				}
			}
		}
				...
				while (p.gold > 0 && !stop) {
					...
					for (int i = 6; i >= 0; i--) { //Buy Units
						if (p.gold >= units[i].cost && rand() % 100 < unit_priority[i]) {
							...
							map[c.y][c.x].unit.type = unit_selected + 1;
							map[c.y][c.x].unit.owner = p.turn;
							map[c.y][c.x].unit.HP = units[unit_selected].HP;
