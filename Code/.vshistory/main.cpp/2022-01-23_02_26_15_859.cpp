...
public:
	...
	void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY, int zom = 16, bool borders = false, bool extra = true) {
		...
		if (zom >= 16) {
			...
			if (tH.y == y && tH.x == x) {
				DrawPartialDecal(olc::vf2d(dX, dY), flat, olc::vf2d(416, 16), olc::vf2d(16, 16), olc::vf2d(z, z), olc::);
			}
			if (tS.y == y && tS.x == x) {
				DrawPartialDecal(olc::vf2d(dX, dY), flat, olc::vf2d(16, 16), olc::vf2d(16, 16), olc::vf2d(z, z));
			}
		}
		else {
