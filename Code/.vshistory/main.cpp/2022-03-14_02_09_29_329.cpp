		...
		if (drawMap && map.size() == MAPSIZE) {
			...
			if (makeCiv > -1) {
				...
				if (makeCiv == 0) {
					...
					if (points <= 0) {
						Print("*RED*" + to_str(points)"*RED*", ScreenWidth() / 2 + 60 * scale, dY + scale + 6 * scale, true, 2);
					}
					else {
						Print("*GREEN*"+ to_str(points) "*GREEN*", ScreenWidth() / 2 + 60 * scale, dY + scale + 6 * scale, true, 2);
					}
					//Footer Title
