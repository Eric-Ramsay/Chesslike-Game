		...
		if (drawMap && map.size() == MAPSIZE) {
			...
			if (makeCiv > -1) {
				...
				if (makeCiv == 1) { //Units
					...
					//Footer Title
					FillRectDecal(dX + 2 * scale, dY + h - (38 * scale), w - (4 * scale), 9 * scale, UI_Blue2);
					Print("Policies", ScreenWidth() / 2, dY + h - (33 * scale), true, 2);
					...
					Print(policies[hovered].name + " - *GOLD*" + to_str(policies[hovered].cost) + "*GOLD*", ScreenWidth() / 2, dY + h - (43 * scale), true, 2);
