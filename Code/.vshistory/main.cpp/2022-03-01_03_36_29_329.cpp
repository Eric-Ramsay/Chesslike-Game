...
public:
	...
	void DrawBuilding(int type, int dX, int dY, float scale, C c = C(15, 15), olc::Pixel owner = olc::WHITE, olc::Pixel pix = olc::WHITE) {
		...
		if (buildings[index].top_color) {
			DrawPartialDecal(olc::vf2d(dX, dY), building_sprites, olc::vf2d(sX, sY - 16), olc::vf2d(16, 16), olc::vf2d(z, z), owner);
		}
	...
	void DrawDrag() { //Draw unit that is being dragged
		...
		if (dragging_unit) {
			...
			DrawBuilding(buildings[building_selected].type, dX, dY-zoom, C(15, 15) colors[p.turn], pix);
		}
		if (attacking_unit) {
