		...
		if (zom >= 16) {
			...
			/*if (drawCurrentDirection) {
				if (map[y][x].elev == FLAT) {
					...
					case -1: break;
					case 0: color = olc::Pixel(255, 165, 0, intens);  break; //d -> orange
					case 1: color = olc::Pixel(255, 0, 0, intens); break; //r -> red
					case 2: color = olc::Pixel(120, 255, 200, intens);  break; //u -> teal
					case 3: color = olc::Pixel(0, 255, 0, intens);  break; //l -> green
					}
					intens = map[y][x].forest_C;
					if (intens > 255) {
						intens = 255;
					}
					color = olc::Pixel(0, 255, 0, intens);
				}
			}
			//color = olc::Pixel(0, 0, 255, map[y][x].temp * 2.55);*/
