		...
		if (drawMap) {
			...
			if (building_UI) {
				if (inBox(x, y, building_bounds)) {
					UI_Click = true;
					if (GetMouse(0).bPressed) {
						building_selected = min(buildings.size() - 1, (x - building_bounds.x) / (building_bounds.w / 6));
			...
			if (building_UI) { //building purchasing
				...
				for (int i = 0; i < buildings.size(); i++) {
					...
					if (i == 0) {
						...
						DrawBuilding(buildings[i].type, dX, dY - 8 * scale, scale, C(0, 0), colors[p.turn]);
					}
