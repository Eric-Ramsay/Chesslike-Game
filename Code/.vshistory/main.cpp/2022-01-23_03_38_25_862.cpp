	...
	void startTurn() {
		...
		for (int i = p.units.size()-1; i >= 0; i--) {
			...
			if (map[c.y][c.x].unit.owner == p.turn && map[c.y][c.x].unit.type > 0) {
				units.push_back(c);
				if (map[c.y][c.x].building > 0 && map[c.y][c.x].owner != p.turn) { //Claim Building - Idk how this would happen lol
					map[c.y][c.x].owner = p.turn;
					p.buildings.push_back(c);
				}
				map[c.y][c.x].unit.MP = units[map[c.y][c.x].unit.type - 1].MP; //Give unit its MP
				if (map[c.y][c.x].unit.HP < units[map[y][x].unit.type - 1].HP) { //Heal Unit
					if (map[c.y][c.x].building == 5) { //Heal on House
						map[c.y][c.x].unit.HP = min(units[map[y][x].unit.type - 1].HP, map[c.y][c.x].unit.HP + 2);
					}
				}
