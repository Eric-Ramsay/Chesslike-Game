...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override
	{
		...
		if (!UI_Click) {
			...
			if (buying_building) { //Buy a Building
				...
				else if (!GetMouse(0).bHeld && turn == p.turn) {
	...
	void DrawDrag() { //Draw unit that is being dragged
		...
		if (dragging_unit) {
			...
			Draw(armySprites, dX, dY - zoom, units[unit_selected].sX, 15 + units[unit_selected].sY, 16, 17, z, p);
			DrawPartialDecal(olc::vf2d(dX, dY - zoom), armySprites, olc::vf2d(sXunits[unit_selected].sX, sY + 17), olc::vf2d(16, 32), olc::vf2d(z, z), p);
		}
			...
			if (map[y][x].building == 0) {
				...
				if (building_selected == 0) { //farm
					sX = 464;
					DrawPartialDecal(olc::vf2d(dX, dY - zoom), flat, olc::vf2d(sX, sY), olc::vf2d(16, 32), olc::vf2d(z, z), p);
				}
				else { //house
					sX = 480;
					DrawPartialDecal(olc::vf2d(dX, dY - zoom), flat, olc::vf2d(sX, sY), olc::vf2d(16, 32), olc::vf2d(z, z), p);
