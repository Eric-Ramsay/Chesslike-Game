...
private:
	...
	//AUDIO FILES - I should probably make this a vector indexed by an enum
...
public:
	...
	void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY, int zom = 16, bool borders = false, bool extra = true) {
		...
		if (!buying_unit && !buying_building && map[tS.y][tS.x].unit.type > 0) {
			...
			moveCost = moveCost(Spot(tS.x, tS.y), Spot(x, y), MP, map[tS.y][tS.x].unit.owner);
				dotColor = colors[p.turn];
			...
			else if (moveCost(Spot(tS.x, tS.y), Spot(x, y), units[map[tS.y][tS.x].unit.type - 1].MP, map[tS.y][tS.x].unit.owner) <= ) {
				drawDot = true;
			}
		}
		dotColor.a = 100;
