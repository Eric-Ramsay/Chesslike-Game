...
public:
	...
	void DrawUI() {
		...
		if (MM_active) {
			...
			if (map[tH.y][tH.x].building > 0) {
				...
				Print(to_str((int)buildings[map[tH.y][tH.x].HP), dX + 19 * scl, dY + 37 * scl, true, scl);
				FillRectDecal();
			}
		}
		//Off 16 zoom warning
	...
	std::vector<Tile> readRow(std::string in) {
		...
		for (char c : in) {
			...
			case 5: t.building = (c - '0'); break; //Building HP
