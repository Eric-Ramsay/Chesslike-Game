...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override {
			...
			if (!UI_Click) {
				...
				if (dragging_unit) { //Move a Unit
					if (buying_unit || buying_building) {
						...
						if (turn == P_TURN && map[drag.y][drag.x].unit.owner == P_TURN) {
				...
				if (buying_building) { //Buy a Building
					if (buying_unit || dragging_unit) {
						...
						if (turn == P_TURN && canBuyBuilding(tH.x, tH.y, building_selected)) {
				...
				if (buying_unit) { //Buy a Unit
					if (buying_building || dragging_unit) {
						...
						if (turn == P_TURN && canBuy(tH.x, tH.y, unit_selected)) {
							...
							map[tH.y][tH.x].unit.owner = P_TURN;
							map[tH.y][tH.x].unit.HP = getMaxHP(unit_selected+1, players[P_TURN]);
				...
				if (attacking_unit) {
					if (!GetMouse(1).bHeld) {
						...
						if (turn == P_TURN && map[tS.y][tS.x].unit.type > 0 && map[tS.y][tS.x].unit.MP > 0 && map[tS.y][tS.x].unit.owner == P_TURN) {
							if (x_dist <= units[map[tS.y][tS.x].unit.type - 1].range && y_dist <= units[map[tS.y][tS.x].unit.type - 1].range) {
								...
								if (map[tS.y][tS.x].unit.type != MANGONEL && map[tH.y][tH.x].unit.type > 0 && map[tH.y][tH.x].unit.owner != P_TURN) {
									map[tH.y][tH.x].unit.HP -= getATK(map[tS.y][tS.x].unit.type, players[map[tS.y][tS.x].unit.owner]);
								...
								//Fish with the peasant, mayor, or fishing raft
									...
									if (map[tH.y][tH.x].forest == FISH && map[tH.y][tH.x].harvest < harvest) {
										...
										if (map[tS.y][tS.x].unit.MP > 0 && x_dist <= units[map[tS.y][tS.x].unit.type - 1].range && y_dist <= units[map[tS.y][tS.x].unit.type - 1].range) {
											if (map[tS.y][tS.x].unit.owner == P_TURN) {
												players[P_TURN].gold++;
								...
								//Damage a building with your unit, ie attack a building
								else if (map[tH.y][tH.x].building > 0 && map[tH.y][tH.x].owner != P_TURN) { //raid building
									int x_dist = coord_dist(tH.x, tS.x);
									int y_dist = coord_dist(tH.y, tS.y);
									...
									if (map[tS.y][tS.x].unit.MP > 0 && x_dist <= units[map[tS.y][tS.x].unit.type - 1].range && y_dist <= units[map[tS.y][tS.x].unit.type - 1].range) {
										...
										for (int i = -1; i < 2; i++) {
											for (int j = -1; j < 2; j++) {
												if (i != 0 || j != 0) { //check for adjacent enemy forts
													if (map[safeC(tH.y + i)][safeC(tH.x + j)].owner != P_TURN && bIndex(map[safeC(tH.y + i)][safeC(tH.x + j)].building) == FORT) {
														proceed = false;
														i = 2; j = 2;
													}
												}
										...
										if (bIndex(map[tH.y][tH.x].building) == FORT || proceed) {
											//Damage the building
											int dmg = getATK(map[tS.y][tS.x].unit.type, players[P_TURN]);
				...
				if (dragging_crown) {
					...
					if (!GetMouse(0).bHeld) { //Promote Unit to Commander
						if (turn == P_TURN && map[tH.y][tH.x].unit.type > 0 && map[tH.y][tH.x].unit.type != COMMANDER && map[tH.y][tH.x].unit.owner == P_TURN) {
							if (!units[map[tH.y][tH.x].unit.type - 1].naval) {
								if (players[P_TURN].gold >= (16 - units[map[tH.y][tH.x].unit.type - 1].cost)) {
									...
									map[tH.y][tH.x].unit.HP = min(map[tH.y][tH.x].unit.HP + 10, getMaxHP(COMMANDER, players[P_TURN]));
									map[tH.y][tH.x].unit.MP = 0;
									sendData("u" + sendUnit(tH.x, tH.y));
								}
							}
				...
				if (dragging_heal) {
					...
					if (!GetMouse(0).bHeld) { //Heal unit
						if (turn == P_TURN && map[tH.y][tH.x].unit.type > 0 && map[tH.y][tH.x].unit.owner == P_TURN) {
							playSound(CLINK);
							map[tH.y][tH.x].unit.HP = min(map[tH.y][tH.x].unit.HP + 5, getMaxHP(map[tH.y][tH.x].unit.type, players[P_TURN]));
							sendData("u" + sendUnit(tH.x, tH.y));
							can_heal = false;
						}
						dragging_heal = false;
				...
				if (GetMouse(1).bPressed) {
					...
					if (turn == P_TURN && map[tS.y][tS.x].unit.type > 0 && map[tS.y][tS.x].unit.MP > 0) {
						attacking_unit = true;
		...
		if (drawMap && map.size() == MAPSIZE) {
			...
			if (makeCiv > -1) {
				...
				if (makeCiv == 1) { //Units
					...
					Print(to_str(getMaxHP(hovered+1, players[P_TURN])), dX + 17 * 2*scale - 1, dY + 13 * 2*scale, true, 2*scale); //HP		
					Print(to_str(getATK(hovered+1, players[P_TURN])), dX + 34 * 2*scale, dY + 1 * 2*scale, false, 2 * scale); //ATK
					...
					Print(to_str(getMaxMP(hovered+1, P_TURN)), dX + 51 * 2*scale, dY + 9 * 2*scale, false, 2*scale); //MP
				}
			...
			if (clean) {
				...
				for (int i = 0; i < map.size(); i++) {
					for (int j = 0; j < map.size(); j++) {
						if (map[i][j].unit.type > 0 && map[i][j].unit.owner == P_TURN) {
							players[P_TURN].units.push_back(C(j, i));
							old_stuff = true;
						}
						if (map[i][j].building > 0 && map[i][j].owner == P_TURN) {
		...
		if (!drawMap) {
			...
			DrawBuilding(6, dX-80, dY+32, 2, 99, C(10, 73), P_TURN);
			DrawBuilding(9, dX-16, dY+32, 2, 99, C(31, 5), P_TURN);
			DrawBuilding(6, dX+48, dY+32, 2, 99, C(105, 73), P_TURN);
		}
	...
	void startTurn() {
		...
		for (int i = players[P_TURN].units.size()-1; i >= 0; i--) {
			...
			if (map[c.y][c.x].unit.owner == P_TURN && map[c.y][c.x].unit.type > 0) {
				...
				//Check if the unit is next to any mages, heal if so.
				for (int a = -1; a < 2; a++) {
					for (int b = -1; b < 2; b++) {
						if (map[safeC(c.y + a)][safeC(c.x + b)].unit.type == MAGE) {
							map[c.y][c.x].unit.HP += 2;
							a = 2;
							b = 2;
						}
				...
				//Make sure not to go beyond unit's max HP
				map[c.y][c.x].unit.HP = min(getMaxHP(map[c.y][c.x].unit.type, players[map[c.y][c.x].unit.owner]), map[c.y][c.x].unit.HP);
				s += sendUnit(c.x, c.y);
			}
		}
		...
		for (int i = players[P_TURN].buildings.size() - 1; i >= 0; i--) {
			...
			if (map[c.y][c.x].owner == P_TURN && map[c.y][c.x].building > 0) {
				bType = bIndex(map[c.y][c.x].building);
				if (map[c.y][c.x].HP >= getBuildingHP(map[c.y][c.x].building, players[map[c.y][c.x].owner]) / 2.0) {
					...
					else if (map[c.y][c.x].building == 4 && map[c.y][c.x].unit.type > 0 && map[c.y][c.x].unit.owner == P_TURN) { //Collect from farm
						int farm_yield = 1;
				...
				if (map[c.y][c.x].HP < getBuildingHP(map[c.y][c.x].building, players[map[c.y][c.x].owner])) {
					delta = true;
					map[c.y][c.x].HP++;
				...
				if (bType != PIER) { //check adjacent buildings for fire that might spread
					for (int a = -1; a < 2; a++) {
						for (int b = -1; b < 2; b++) {
							if (a != 0 || b != 0) {
								if (map[safeC(c.y + a)][safeC(c.x + b)].building > 0 && map[safeC(c.y + a)][safeC(c.x + b)].HP < getBuildingHP(map[safeC(c.y + a)][safeC(c.x + b)].building, players[map[safeC(c.y + a)][safeC(c.x + b)].owner]) / 2) {
									if (rand() % 2 == 0) {
										map[c.y][c.x].HP -= 2;
										a = 2;
										delta = true;
		...
		for (C c : players[P_TURN].units) {
			...
			if (map[c.y][c.x].unit.type != MERCHANT) {
				...
				if (map[c.y][c.x].building > 0 && map[c.y][c.x].building != STAKES && map[c.y][c.x].building != DEPOT) {
					if (map[c.y][c.x].owner != P_TURN) {
						map[c.y][c.x].owner = P_TURN;
	...
	bool canBuy(int x, int y, int index) {
		...
		if (bIndex(map[y][x].building) == FORT && map[y][x].owner != P_TURN) { //can't build units on enemy forts
			return false;
		}
		if (!s) {
			if (players[P_TURN].policies[VOLUNTEERS] && map[y][x].owner == P_TURN && (bIndex(map[y][x].building) == FORT || bIndex(map[y][x].building) == PIER)) {
				s = true;
			}
			else {
				for (int i = -1; i < 2; i++) {
					for (int j = -1; j < 2; j++) {
						if (map[safeC(y + i)][safeC(x + j)].unit.owner == P_TURN) {
	...
	bool canBuyBuilding(int x, int y, int index) {
		...
		for (int i = -1; i < 2; i++) {
			for (int j = -1; j < 2; j++) {
				if ((map[safeC(y + i)][safeC(x + j)].unit.type == PEASANT || map[safeC(y + i)][safeC(x + j)].unit.type == MAYOR) && map[safeC(y + i)][safeC(x + j)].unit.owner == P_TURN) {
					s = true;
					i = 2; j = 2;
				}
			}
		...
		s = s && (map[y][x].building == 0 || (GetKey(olc::Key::SHIFT).bHeld && map[y][x].owner == P_TURN));
	...
	void DrawUI() {
		...
		if (MM_active) {
			...
			Draw(icons, 4 + 49*scale, 2+6*scale, 112, 0, 11, 11, scale, colors[P_TURN]); //unit color
			...
			Draw(icons, ScreenWidth() - (2 + 31 * scale) + 2 * scale, 2 + 2 * scale, 2, 34, 11, 11, scale, colors[P_TURN]); //Player colors 2 34
			...
			int cost = 16;
			if (map[tH.y][tH.x].unit.owner == P_TURN && map[tH.y][tH.x].unit.type > 0) {
			...
			if (unit_UI) { //Unit Purchasing
				...
				for (int i : UI_Units) {
					...
					Draw(armySprites, dX + 2 * scale, dY + 2*scale, u.sX, 17 + u.sY, 16, 14, scale, colors[P_TURN]); //Draw Unit's colors
					//Print Cost
					Print(to_str(u.cost), dX + 35 * scale, dY + 6 * scale, true, scale);
					//HP
					Print(to_str(getMaxHP(i+1, players[P_TURN])), dX + 35 * scale, dY + 14 * scale, true, scale);
					//ATK
					Print(to_str(getATK(i+1, players[P_TURN])), dX + 53 * scale, dY + 2 * scale, false, scale);
					//DEF
					Print(to_str(u.DEF), dX + 53 * scale, dY + 10 * scale, false, scale);
					//RANGE
					Print(to_str(u.range), dX + 70 * scale, dY + 2 * scale, false, scale);
					//MP
					Print(to_str(getMaxMP(i+1, players[P_TURN])), dX + 70 * scale, dY + 10 * scale, false, scale);
					if (i == unit_selected) {
						selected_dY = dY;
					}
					dY += 18 * scale;
			...
			if (building_UI) { //building purchasing
				...
				for (int i = 0; i < buildings.size(); i++) {
					Draw(building_sprites, dX, dY, sX, sY, sW, sH, scale);
					if (i == 0) {
						DrawBuilding(4, dX+scale, dY - 8 * scale, scale, 99, C(0, 0), P_TURN);
					}
					else {
						DrawBuilding(buildings[i].type, dX+scale, dY - 8 * scale, scale, 99, C(0, 0), P_TURN);
					}
			...
			if (map[tH.y][tH.x].unit.type != NO_UNIT) {
				...
				Print(to_str(getATK(map[tH.y][tH.x].unit.type, players[map[tH.y][tH.x].unit.owner])), dX+55*scl, dY, true, scl); //ATK
			...
			if (map[tH.y][tH.x].building > 0) {
				...
				int hp = 29*2 * (float)map[tH.y][tH.x].HP / getBuildingHP(map[tH.y][tH.x].building, players[map[tH.y][tH.x].owner]);
	...
	void DrawBuilding(int type, int dX, int dY, float scale, int HP = 99, C c = C(15, 15), int owner = 0, olc::Pixel pix = olc::WHITE) {
		...
		if (HP < getBuildingHP(type, players[owner])/2.0) { //draw fire sprite
			sX = 48;
			sY = 48;
	...
	void DrawDrag() { //Draw unit that is being dragged
		...
		if (dragging_unit) {
			...
			int hp = 12 * ((float)map[y][x].unit.HP / getMaxHP(map[y][x].unit.type, players[map[y][x].unit.owner]));
			int mov = 12 * ((float)map[y][x].unit.MP / getMaxMP(map[y][x].unit.type, players[map[y][x].unit.owner]));
			//			     x  y sX  sY  w   h  scale
			...
			Draw(armySprites, dX, dY - (zoom-z), units[unit_selected].sX, 17 + units[unit_selected].sY, 16, 14, z, colors[P_TURN]);
		}
			...
			DrawBuilding(buildings[building_selected].type, dX, dY-zoom, z, 99, C(15, 15), P_TURN, pix);
		}
		if (attacking_unit) {
			if (map[tS.y][tS.x].unit.type > 0 && map[tS.y][tS.x].unit.owner == P_TURN && map[tS.y][tS.x].unit.MP > 0) {
	...
	void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY, int zom = 16, bool borders = false, bool extra = true) {
		...
		if (!buying_unit && !buying_building && map[tS.y][tS.x].unit.type > 0) {
			...
			if (map[tS.y][tS.x].unit.owner != turn) {
				MP = getMaxMP(map[tS.y][tS.x].unit.type, players[map[tS.y][tS.x].unit.owner]);
			}
		...
		if (zom >= 16) {
			...
			olc::Pixel players[P_TURN](255, 255, 255);
			switch (map[y][x].type) {
			case OCEAN: players[P_TURN] = Water; break;
			case RIVER: players[P_TURN] = River; break;
			case LAKE: players[P_TURN] = Lake; break;
			case GRASS: players[P_TURN] = Grass; break;
			case STEPPE: players[P_TURN] = Steppe; break;
			case SAVANNA: players[P_TURN] = Savanna; break;
			case COLD_DESERT: players[P_TURN] = Cold_Desert; break;
			case DESERT: players[P_TURN] = Desert; break;
			case TUNDRA: players[P_TURN] = Tundra; break;
			case ICE: players[P_TURN] = Ice; break;
			case BOG: players[P_TURN] = Bog; break;
			case MEADOW: players[P_TURN] = Meadow; break;
			case SHALLOW: players[P_TURN] = Ocean; break;
			}
			if (map[y][x].forest != NONE) {
				players[P_TURN] = Forest;
			}
			else if (map[y][x].elev == HILL) {
				players[P_TURN] = Hill;
			}
			else if (map[y][x].elev == MOUNTAIN) {
				players[P_TURN] = Mountain;
			}
			/*if (drawCurrentDirection) {
				if (map[y][x].elev == FLAT) {
					...
					case -1: break;
					case 0: players[P_TURN] = olc::Pixel(255, 165, 0, intens);  break; //d -> orange
					case 1: players[P_TURN] = olc::Pixel(255, 0, 0, intens); break; //r -> red
					case 2: players[P_TURN] = olc::Pixel(120, 255, 200, intens);  break; //u -> teal
					case 3: players[P_TURN] = olc::Pixel(0, 255, 0, intens);  break; //l -> green
					}
					intens = map[y][x].forest_C;
					if (intens > 255) {
						intens = 255;
					}
					players[P_TURN] = olc::Pixel(0, 255, 0, intens);
				}
			}
			//players[P_TURN] = olc::Pixel(0, 0, 255, map[y][x].temp * 2.55);*/
			FillRect(olc::vf2d(dX, dY), olc::vf2d(zom, zom), players[P_TURN]);
			if (map[y][x].owner != 0) {
				olc::Pixel players[P_TURN] = colors[map[y][x].owner];
				players[P_TURN].a = 75;
				FillRect(olc::vf2d(dX, dY), olc::vf2d(zom, zom), players[P_TURN]);
			}
		}
	...
	int Print(std::string text, int x = -1, int y = -1, bool center = false, float scale = 1.0f, int width = 1920/2) {
		int cap = width;
		olc::Pixel players[P_TURN] = WHITE;
		if (y == -1) {
			cap = x;
		}
		...
		for (char& c : text) {
			...
			if (c == '*') {
				skipping = !skipping;
				if (!skipping) {
					if (GetColor(color, timer) != players[P_TURN]) {
						players[P_TURN] = GetColor(color, timer);
					}
					else {
						players[P_TURN] = WHITE;
					}
					color = "";
				}
			}
				...
				if (prnt) {
					DrawPartialDecal(olc::vf2d(drawX + offX * scale, drawY + offY * scale), olc::vf2d(sW * scale, sH * scale), font, olc::vf2d(sX, sY), olc::vf2d(sW, sH), players[P_TURN]);
				}
				if (c != '\n') {
	...
	void client_listen() {
		...
		while (connected) {
			...
			if (bytesReceived > 1) {
				...
				char sender = msg[2] - '0';
				static int num = 0;
				bool getMessage = (sender == 0) || (context == 0 && sender == P_TURN) || (context == 1 && sender != P_TURN);
				...
				if (P_TURN == 0) {
					...
					if (c == 'r') { //Get Row
						...
						turn = std::stoi(msg);
						if (turn == P_TURN && drawMap) {
							startTurn();
							playSound(CHIMES);
						}
					}
					else if (c == 'players[P_TURN]') { //policies
						int p_turn = sender;
