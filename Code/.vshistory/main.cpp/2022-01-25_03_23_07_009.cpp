...
public:
	...
	bool OnUserUpdate(float fElapsedTime) override {
		...
		if (!UI_Click) {
			...
			if (dragging_crown) {
				...
				if (!GetMouse(0).bHeld) { //Promote Unit to Commander
					if (turn == p.turn && map[tH.y][tH.x].unit.type > 0 && map[tH.y][tH.x].unit.type != 7 && map[tH.y][tH.x].unit.owner == p.turn) {
						if (p.gold >= (16 - units[map[tH.y][tH.x].unit.type - 1].cost)) {
							map[tH.y][tH.x].unit.type = 7;
		...
		Print(to_str(x), ScreenWidth() / 2, ScreenHeight() / 2, 3);
		Print(to_str(y), ScreenWidth() / 2, ScreenHeight() / 2 + 25, 3);
		Print(to_str(crown.x), ScreenWidth() / 2, ScreenHeight() / 2 + 50, 3);
		Print(to_str(crown.y), ScreenWidth() / 2, ScreenHeight() / 2 + 75, 3);
