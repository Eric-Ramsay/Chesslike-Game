		...
		if (attacking_unit) {
			if (true || (map[tS.y][tS.x].unit.type > 0 && map[tS.y][tS.x].unit.owner == p.turn)) {
				...
				if (sFound) {
					...
					DrawPartialRotatedDecal(olc::vf2d(sX + 8 * z, sY + 8 * z), icons, angle, olc::vf2d(8, 16), olc::vf2d(80, 80), olc::vf2d(16, 16), olc::vf2d(1.0, dist * 2 * z), colors[map[y][x].unit.owner]);
				}
			}
		}
	}
