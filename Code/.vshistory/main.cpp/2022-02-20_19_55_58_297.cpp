	...
	void startTurn() {
		...
		for (int i = 0; i < MAPSIZE; i++) { //build the encyclopedia
			for (int j = 0; j < MAPSIZE; j++) {
				if (map[i][j].unit.type > 0 && map[i][j].unit.owner > 0) {
					int found_index = -1;
					for (int a = 0; a < encyclopedia.size(); a++) {
						if (encyclopedia[a].turn == map[i][j].unit.turn) {
							found_index = a;
							a = encyclopedia.size();
						}
					}
					...
					encyclopedia[found_index].unit_value += units[map[i][j].unit.type - 1].value;
				}
			}
		}
	}
		...
		if (MM_active) {
			...
			if (draw_encyclopedia) {
				...
				for (Entry e : encyclopedia) {
					...
					//Draw player color
					Draw(icons, dX, dY, 128, 3, 7, 7, scale, colors[e.turn]);
					Draw(icons, ScreenWidth() - 22, ScreenHeight() - 40, 32, 32, 10, 10, 2, colors[turn]);
					//Print income and unit value
					Print(e.income, dX + 17 * scale, dY + 3 * scale , true, scale);
					Print(e.unit_value, dX + 33 * scale, dY + 3 * scale , true, scale);
				}
				//Draw the bottom
