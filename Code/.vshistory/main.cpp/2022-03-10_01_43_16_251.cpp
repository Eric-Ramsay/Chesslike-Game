...
public:
		...
		if (MM_active) {
			...
			if (unit_UI> 0) { //Unit Purchasing
				...
				for (int i = unit_start; i <= unit_end; i++) {
					...
					Draw(armySprites, dX + 2 * scale, dY + 2*scale, u.sX, 18 + u.sY, 16, 16, scale, colors[p.turn]); //Draw Unit's colors
					//Print Cost
	...
	void DrawDrag() { //Draw unit that is being dragged
		...
		if (dragging_unit) {
			...
			//  x  y sX  sY  w   h  scale
			DrawPartialDecal(olc::vf2d(dX, dY), armySprites, olc::vf2d(sX, sY + 18), olc::vf2d(16, 16), olc::vf2d(z, z), ap); //Draw Color
	...
	void DrawTile(std::vector<std::vector<Tile>>& map, int x, int y, int dX, int dY, int zom = 16, bool borders = false, bool extra = true) {
		...
		if (zom >= 16) {
			...
			if (map[y][x].unit.type > 0 && !(dragging_unit && drag.x == x && drag.y == y)) { //Unit on Tile
				...
				//  x  y sX  sY  w   h  scale
				DrawPartialDecal(olc::vf2d(dX, dY - ((off-1)*z)), armySprites, olc::vf2d(sX, sY+18), olc::vf2d(16, 16), olc::vf2d(z, z), ap);
