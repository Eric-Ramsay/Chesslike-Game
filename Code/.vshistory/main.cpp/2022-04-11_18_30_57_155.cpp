	...
	bool OnUserUpdate(float fElapsedTime) override {
		...
		if (drawMap && map.size() == MAPSIZE) {
			...
			if (makeCiv > -1) {
				...
				//Body
				FillRectDecal(dX + 2 * scale, dY + 11 * scale, w - 4 * scale, 118 * scale, UI_Blue3);
				//Footer
				FillRectDecal(dX + 2 * scale, dY + h - 35 * scale, w - 4 * scale, 38 * scale, UI_Blue4);

				//Draw Done button
				Draw(icons, dX + 134 * scale, dY + h - 1 * scale, 64, 112, 29, 11, scale);
