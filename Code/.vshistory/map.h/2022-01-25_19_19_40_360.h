	...
	for (int i = 0; i < MAPSIZE; i++) {
		for (int j = 0; j < MAPSIZE; j++) {
			...
			if (t.elevation > 0) {
				...
				MapTile t = map[i][j];
				int treeChance = -100 + t.forestChance * std::max(.5, std::min(1.25, min(200.0 + pow(latTemp(i) / 50, 7), map[i][j].rain) / (200.0 + t.elevation/2 + t.heat/4)));
				map[i][j].forestChance = (t.rain/4 + t.forestChance/2 + t.heat/8) * std::max(.1, fmin(300.0, map[i][j].rain) / (275.0 + t.elevation));
				if (t.rain > 300) {
