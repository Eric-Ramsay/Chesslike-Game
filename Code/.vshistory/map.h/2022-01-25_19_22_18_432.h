...
#include <iostream> //used only in debugging
int NUM_THREADS = 4;
						...
						for (int b = -1; b < 2; b++) {
							if (a * b == 0 && a != b) {
								...
								if (map[i][j].elevation > 0) {
									...
									avg += std::min(0, (int)map[safeC(i + a)][safeC(j + b)].elevation);
								}
							}
						}
					}
	...
	for (int i = 0; i < MAPSIZE; i++) {
		...
		for (int j = 0; j < MAPSIZE; j++) {
			if (map[i][j].elevation > 25 && map[i][j].elevation < 75) {
				int chance = 990 - std::min(map[i][j].rain / 15.0, 20.0);
				if ((rand() % 1000) > chance) {
					makeRiver(i, j);
					id++;
				}
	...
	for (int i = 0; i < MAPSIZE; i++) {
		for (int j = 0; j < MAPSIZE; j++) {
			if (map[i][j].elevation > 35 && map[i][j].elevation < 40) {
				double chance = 1.002 - std::min(map[i][j].rain / 60000.0, .005);
	...
	for (int i = 0; i < MAPSIZE; i++) {
		for (int j = 0; j < MAPSIZE; j++) {
			...
			if (t.elevation > 0) {
				...
				MapTile t = map[i][j];
				int treeChance = -100 + t.forestChance * std::max(.5, (float)std::min(1.25, (float)std::min(200.0 + pow(latTemp(i) / 50, 7), map[i][j].rain) / (200.0 + t.elevation/2 + t.heat/4)));
				map[i][j].forestChance = (t.rain/4 + t.forestChance/2 + t.heat/8) * std::max(.1, std::min(300.0, map[i][j].rain) / (275.0 + t.elevation));
				if (t.rain > 300) {
				...
				if (map[i][j].rain > 195) {
					treeChance = std::max(treeChance, 4);
				}
				if (rand() % 100 < treeChance) {
					if (map[i][j].rain < 250) {
						map[i][j].forest = 'l';
...
std::vector<std::vector<Tile>> createMap(int seed, std::vector<std::vector<MapTile>> m = {}) {
	srand(seed);
	bool makeMountains = false;
	std::vector<std::vector<MapTile>> map = m;
	if (m.size() == 0) {
		makeMountains = true;
		map.resize(MAPSIZE);
		for (int i = 0; i < MAPSIZE; i++) {
